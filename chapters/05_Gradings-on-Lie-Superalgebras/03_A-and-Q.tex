...
Define $\theta_0 \coloneqq \psi_0 \vphi_0 = \vphi_0 \psi_0$ and $\Theta \coloneqq \Lambda\Phi$. 
Then:
\[\label{eq:theta-with-matrix-1}
    \theta(X) = \Theta\inv\, \theta_0(X)\stransp \, \Theta,
\]
for all $X \in M_k(\D)$. 
...
\]
where $\Theta \in M_k(\barr \D)$ is defined by $\Theta_{ij} \coloneqq \chi(g_i) B(u_i, u_j) \epsilon$, and $\theta_0$ is the super-anti-automorphism on $\barr \D$ associated to $\bar \mu$. 
...
In particular, $\kappa\from G^\# \to \ZZ_{\geq 0}$ corresponds to a pair $(\kappa_\bz, \kappa_\bo)$ (see \cref{ssec:supermodules-over-D}). 
...
\begin{defi}\label{def:A-Type-I}
    ...
    We will denote by $\Gamma^{\mathrm{(I)}}_A(T, \beta, \kappa_\bz, \kappa_\bo)$ the restriction to $L$ of the grading $\Gamma_M(T, \beta, \kappa_\bz, \kappa_\bo)$ on $S$ (see \cref{def:Gamma-T-beta-kappa-even}). 
\end{defi}
...
Hence, by \cref{thm:MxM-even}, a Type II grading on $S$ corresponds to a grading on $(R, \vphi)$ making it isomorphic to $M^{\mathrm{ex}} (T, \beta, \kappa_\bz, \kappa_\bo, g_0)$ (\cref{def:model-grd-MxM-even}), where $T$ is an elementary $2$-group, $\beta\from T\times T \to \pmone$ is an alternating bicharacter with $\rad \beta = \langle f \rangle$ for some $f \in T$, $g_0$ is an element in $G^\#$, and $\kappa_\bz, \kappa_\bo \from G/T \to \ZZ_{\geq 0}$ are $g_0$-admissible maps (see \cref{inertia-even-and-odd-case}) such that $|\kappa_\bz| \sqrt{|T|/2} = m+1$ and $|\kappa_\bo| \sqrt{|T|/2} = n+1$. 
...
For each $i \in \ZZ_2$, set $k_i \coloneqq |\kappa_i|$, let $\gamma_i$ be the $k_i$-tuple of elements in $G$ realizing $\kappa_i$ governed by $\xi$ and $\leq$ (see \cref{defi:tuple-governed}), and let $\bar \gamma_i$ be the tuple of elements in $\barr G$ consisting of the images under $\pi\from G\to \barr G$ of the entries of $\gamma_i$ (\ie, $\barr \gamma_i$ is the $k_i$-tuple realizing $\kappa$ governed by $\pi \circ \xi$ and $\leq$). 
...
\begin{defi}\label{defi:blocks-of-Theta}
	...
	\end{enumerate}
    %
    If $g_0 x^2 \neq T$, we define $\Theta(i, x)$ to be the following $2\kappa_i(x) \times 2\kappa_i(x)$-matrix with entries in $\barr\D$:
    %
    \begin{enumerate}[(i)]
        %
        \setcounter{enumi}{2}
...
Let us summarize what have been done for future reference:

\begin{defi}\label{defi:type-II-A-m-not-n}
    Let $m,n \in \ZZ_{\geq 0}$. 
    ...
    Then construct tuples $\bar\gamma_\bz$ and $\bar\gamma_\bo$ realizing $\kappa_\bz$ and $\kappa_\bo$, respectively, governed by $\pi \circ \xi$ and $\leq$ (\cref{defi:tuple-governed}). 
    Consider the $\barr G$-grading $\Gamma_M(\barr T, \barr \beta, \kappa_\bz, \kappa_\bo)$ on $S \coloneqq M(m+1,n+1)$ constructed using the choices of $\barr \D$, $\barr \gamma_\bz$ and $\barr \gamma_\bo$ above (see \cref{def:Gamma-T-beta-kappa-even}), and consider its restriction $\Gamma : L = \bigoplus_{\bar g \in \barr G} L_{\bar g}$ to $L \coloneqq S^{(1)}/Z(S^{(1)})$. 
...
\begin{proof}
    ...
    The choices in items (i) and (ii) give us a way to make to the choices for a standard realization $(\D, \vphi_0)$ as in \ref{def:std-realization-MxM-QxQ}(a).
    ...
    Also, by \cref{lemma:transp-std-realization}, $\mu (ab) = \beta(a,b)$ for all $a\in A$ and $b\in B$, so $\mu\restriction_{K}$ is the map determining $\vphi_{\mc M}$. 
    ...
    We can use the choices in items (iii) and (iv) to choose the pair $(\U, B)$ as in \cref{def:model-grd-MxM-even}, by following the same construction done in \cref{ssec:grds-osp}: Let $\gamma_\bz = (g_1, \ldots, g_{k_\bz})$ and $\gamma_\bo = (h_1, \ldots, h_{k_\bo})$ be the tuples realizing $\kappa_\bz$ and $\kappa_\bo$, respectively, governed by $\xi$ and $\leq$ (\cref{defi:tuple-governed}), and define $\U\even \coloneqq \D^{[g_1]} \oplus \cdots \oplus \D^{[g_{k_\bz}]}$, $\U\odd \coloneqq \D^{[h_1]} \oplus \cdots \oplus \D^{[h_{k_\bo}]}$ and $\U \coloneqq \U\even \oplus \U\odd$. 
    Let $\{u_1, \ldots, u_k \}$ be the canonical graded basis for $\U$. 
    Then define the matrix $\Phi \in M_{k_\bz | k_\bo}(\D)$ as in \cref{eq:puting-the-blocks-of-Phi-together} and use it to define the $\vphi_0$-sesquilinear form $B\from \U\times \U \to \D$ by $B(u_i, u_j) \coloneqq \Phi_{ij}$. 
    
    To get the matrix $\Theta$ as in \cref{eq:puting-the-blocks-of-Phi-together-version-A}, we will consider a different graded basis for $\U$: if $g_0 (\deg u_i)^2 \in T$ (\ie, $u_i$ belongs to a self-dual isotypic component), we define $\tilde u_i \coloneqq \sqrt{\chi(\deg u_i)\inv}$; if $(\deg u_i) T < g_0\inv (\deg u_j)\inv T$, then let $\tilde u_i \coloneqq \chi(\deg u_i)\inv u_i$; otherwise, define $\tilde u_i \coloneqq u_i$. 
    Then let $\tilde \Phi$ be the matrix representing $B$ with respect to the graded basis $\{\tilde u_1, \ldots, \tilde u_k \}$. 
    Following the procedure in \cref{ssec:undoubling} with $\tilde \Phi$ playing the role of $\Phi$, we get that $\Lambda \tilde \Phi$ corresponds to $\Theta$ as above. 
...
Should it be before or after $A(n,n)$? 

It reduces to $A(n,1)$ by corollary in Chapter 4.

% ---------------------------

\subsection{Gradings on \texorpdfstring{$A(n,n)$}{A(n,n)}}\label{ssec:grds-on-Ann}

We now go to our final case of gradings on $A(n,n)$. 
Let $S = M(n+1, n+1)$,  
so $L = \mathfrak{psl} (n+1, n+1) = S^{(1)}/Z(S^{(1)})$. 

Different from \cref{ssec:grds-on-A-m-n}, the gradings on $S$ are separated into two classes: even and odd gradings. 
The gradings on $L$ induced by even gradings on $S$ will be called gradings of \emph{Type I\textsubscript{M}}, and the gradings on $L$ induced by odd gradings on $S$ will be called gradings of \emph{Type I\textsubscript{Q}}. 

\begin{defi}\label{def:Type-I_M}
    Let $n\in \ZZ_{> 0}$ and set $m \coloneqq n$. 
    We define $\Gamma^{\mathrm{(I)_M}}_A(T, \beta, \kappa_\bz, \kappa_\bo)$ to be the grading on $L$ induced by the grading $\Gamma^{\mathrm{(I)}}_A(T, \beta, \kappa_\bz, \kappa_\bo)$ on $M(n+1, n+1)^{(1)}$ (\cref{def:A-Type-I}).
\end{defi}
...
\begin{defi}
    ...
    We will denote by $\Gamma^{\mathrm{(I)_M}}_A(T, \beta, \kappa_\bz, \kappa_\bo)$ the restriction to $L$ of the grading $\Gamma_M(T, \beta, \kappa_\bz, \kappa_\bo)$ on $S$ (see \cref{def:Gamma-T-beta-kappa-even}).
\end{defi}

To parametrize the gradings of Type I\textsubscript{Q}, we recall the parametrization of odd gradings on $S$ only in terms of the group $G$ (\cref{sec:assc-only-G}, \cref{cor:iso-odd-M-simplified}). 
The character $\chi_0 \in \widehat{T^+}$ in the next definition plays the role of $\chi$ there. 

\begin{defi}\label{def:Type-I_Q}
    Let $T^+ \subseteq G$ be a finite subgroup, let $\beta^+\from T^+ \times T^+ \to \FF^\times$ be an alternating bicharacter with $\rad \beta^+ = \langle t_p \rangle$, where $t_p\in T^+$ is an element of order $2$, and let $\kappa \from G/T \to \ZZ_{\geq 0}$ be a map with finite support such that $|\kappa| \sqrt{|T|} = n+1$. 
    Choose a character $\chi_0 \in \widehat{T^+}$ such that $\chi_0 (t_p) = -1$. 
    Then let $a \in T^+$ be the unique element such that $\chi^2 = \beta^+(a, \cdot)$ and $\chi(a) = 1$ (see \cref{lemma:chi-defines-a}). 
    ...
    Let $p\from T \to \ZZ_2$ be the restriction of the projection onto the second component of $G^\# = G\times \ZZ_2$, and let $\beta\from T \times T \to \FF^\times$ be the unique alternating bicharacter such that $\beta\restriction_{T^+ \times T^+} = \beta^+$ and $\beta(t_1, t) = \chi(t)$, for all $t\in T^+$ (see \cref{lemma:existence-beta}). 
    Let $\kappa \from G/T \to \ZZ_{\geq 0}$ be a map with finite support such that $|\kappa| \sqrt{|T|} = n+1$. 
    We will denote by  $\Gamma^{\mathrm{(I)_Q}}_A(T^+, \beta^+, h, \kappa)$ the restriction to $L$ of the grading $\Gamma_M(T, \beta, p, \kappa)$ on $S$ (see \cref{def:Gamma-T-beta-kappa-odd}). 
\end{defi}

Recall that a element $h \in G$ (and, hence, of a grading $\Gamma^{\mathrm{(I)_Q}}_A(T^+, \beta^+, h, \kappa)$) exists \IFF $\theta(T^+_{[2]})^\perp \subseteq \barr G^{[2]}$ (see \cref{def:A_[m],defi:perp,prop:O_M-non-empty}).

The nomenclature is justified by the following:

\begin{prop}\label{prop:Q-implies-GxZZ2-grading-on-M}
    Every $G$-grading on $Q(n)$ induces a $G\times \ZZ_2$-grading on $M(n,n)$. 
\end{prop}

SHOULD IT BE IN CHAPTER II?

The Type II gradings on $L$ will also be subdivided in into different types. 
...
\begin{defi}\label{def:type-II-osp}
    Let $n\in \ZZ_{> 0}$ and set $m \coloneqq n$. 
    We define $\Gamma^{\mathrm{(II)_osp}}_A(T, \beta, \kappa_\bz, \kappa_\bo, g_0)$ to be the grading on $L = R^{(1)}/Z(R^{(1)})$ induced by the grading $\Gamma^{\mathrm{(II)}}_A(T, \beta, \kappa_\bz, \kappa_\bo)$ on $M(n+1, n+1)^{(1)}$ (\cref{defi:type-II-A-m-not-n}).
\end{defi}

To construct a model for gradings of Type II\textsubscript{P}, we follow a similar approach as for Type II\textsubscript{osp}, but it is simpler since we do not need to choose $\xi$ and $\leq$ to define the matrix $\Theta$ (in the same way this choices were needed in \cref{ssec:grds-osp} but not in \cref{ssec:grds-P(n)}). 

\begin{defi}\label{def:type-II-P}
    Let $n\geq 2$. 
    Let $T \subseteq G$ be a finite $2$-elementary subgroup, let $\beta\from {T\times T} \to \FF^\times$ be an alternating bicharacter with $\rad \beta = \langle f \rangle$, for some order $2$ element $f\in T$, and let $h_0 \in G$. 
    Set $g_0 \coloneqq (h_0, \bar 1) \in G^\#$, $\barr G \coloneqq G/\langle f \rangle$, $T \coloneqq T/\langle f \rangle$, and let $\bar \beta$ be the nondegenerate alternating bicharacter on $\barr T$ induced by $\beta$. 
    Choose:
    \begin{enumerate}[(i)]
        \item a standard realization $\barr \D$ associated to $(\barr T, \barr \beta)$; 
        \item a subgroup $K \subseteq T$ such that $T = K \times \langle f \rangle$.
    ...
    Let $\kappa_\bz, \kappa_\bo \from G/T \to \ZZ_{\geq 0}$ be $g_0$-admissible maps (\cref{inertia-even-and-odd-case}) such that $n+1 = k \sqrt{|T|/2}$, where $k \coloneqq |\kappa_\bz| = |\kappa_\bo|$ (recall that $g_0$ is odd). 
    Then choose:
    \begin{enumerate}[(i)]
        \setcounter{enumi}{2}
        \item a $k$-tuple $\gamma_\bz = (g_1, \ldots, g_k)$, of elements in $G$, realizing $\kappa_\bz$.
    \end{enumerate}
    Set $\gamma_\bo \coloneqq (h_0\inv g_1\inv, \ldots, h_0\inv g_k\inv)$ and set $\bar \gamma_\bz, \bar \gamma_\bo$ accordingly..  
    Consider the $\barr G$-grading $\Gamma_M(\barr T, \barr \beta, \kappa_\bz, \kappa_\bo)$ on $S \coloneqq M(n+1,n+1)$ using the choices above (see \cref{def:Gamma-T-beta-kappa-even}), and consider its restriction $\Gamma : L = \bigoplus_{\bar g \in \barr G} L_{\bar g}$ to $L \coloneqq S^{(1)}$. 
    Consider ${\Theta \in S}$ 
    given by
    ...
    Let $T \subseteq G$ be a finite $2$-elementary subgroup, let $\beta\from {T\times T} \to \FF^\times$ be an alternating bicharacter such that $\rad \beta = \langle f \rangle$, with $e \neq f\in T$, let $h_0 \in G$ and let $\kappa_\bz\from G/T \to \ZZ_{\geq 0}$ be a map with finite support such that $k \sqrt{|T|/2} = n+1$, where $k \coloneqq |\kappa_\bz|$. 
...
The proof it works should be the same.

We now go to the last case: odd type II. 
In this case parameters $R$ are bla...
We can use $\chi_1$ to move it to $G$. 
