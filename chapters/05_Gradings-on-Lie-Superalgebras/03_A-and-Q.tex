...
To summarize, once $\chi \in \widehat{T}$ is fixed, the undoubled model is determined by the $\barr G$-graded superalgebra $M_k(\barr \D)$ with parameters given by \cref{prop:lemma-for-undoubling-and-fine-gradings}, the map $\mu \from \barr T \to \FF^{\times}$ defined by \cref{eq:defi-mu-undoubled}, which is
...
\begin{proof}
    ...
    We need to show that $\mu$ as defined in \cref{eq:defi-mu-undoubled} is the map associated to the queer supertranspose. 
    This follows from 
