...
\begin{cor}
    ...
    Let $\vphi, \vphi'$ be degree preserving super-anti-automorphisms on $R, R'$ determined by pairs $(\vphi_0, B), (\vphi_0', B')$. 
    If $\psi\from (R, \vphi) \to (R', \vphi')$ is an isomorphism, then there are $g\in G^\#$, $0\neq d\in \D$ a homogenous element, an isomorphism $\psi_0\from \D \to \D'$, and an isomorphism
    \[
        \psi_1 \from (\U^{[g]}, dB) \to ( (\U')^{\psi_0}, \psi_0\inv \circ B ) 
    \] 
    such that $\psi_1 (r u) = \psi(r) \psi_1 (u)$, for all $r\in R$ and $u\in \U$.
\end{cor}
