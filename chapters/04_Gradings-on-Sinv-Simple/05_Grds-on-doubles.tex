...
\begin{thm}\label{thm:MxM-type-II}
...
\begin{thm}\label{thm:QxQ-type-II}
...
\begin{defi}\label{def:std-realization-MxM-QxQ}
    ...
    \begin{enumerate}[(i)]
        ...
        \item a subgroup $S \subseteq T^+$ such that $(\rad \beta^+) \times S = T^+$ and, if we are in case (b), $\beta(t_1, S) = 1$;
        \label{item:S-can-be-orthogonal-to-t_1}
        \item a standard realization $\mc M$ (see     \cref{def:standard-realization}) of a matrix algebra with a division grading  associated to $(S, \beta\restriction_{S\times S})$. 
        \label{item:choose-mc-M}
    \end{enumerate}
    A subgroup $S$ in item \eqref{item:S-can-be-orthogonal-to-t_1} exists because $T^+$ is an elementary $2$-group and, in case (b), the choice of $S$ satisfying $\beta(t, S) = 1$ is possible by the same argument as in the proof of \cref{thm:tensor-prd-decomposition}. 
    A standard realization in item \eqref{item:choose-mc-M} is defined since $\beta\restriction_{S\times S} = \beta^+\restriction_{S\times S}$ is, clearly, nondegenerate. 
