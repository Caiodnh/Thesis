...
We can use $\gamma$ to construct the graded $\D$-module $\U \coloneqq {}^{[g_1]}\D \oplus \cdots \oplus {}^{[g_k]}\D$, which is clearly a representative of the isomorphism class of graded $\D$-modules determined by $\kappa$. 
...
A systematic way to obtain $\gamma$ from $\kappa$ is using a set-theoretic section $\xi\from G/T \to G$ of the natural homomorphism $\pi\from G \to G/T$ (\ie, a map such that $\pi(\xi(x)) = x$ for all $x \in G/T$) and a total order $\leq$ on $G/T$. 
...
\begin{defi}\label{defi:tuple-governed}
    ...
    Given a map $\kappa\from G/T \to \ZZ_{\geq 0}$ with finite support, set $k \coloneqq |\kappa|$ and let $\gamma$ be the $k$-tuple given by putting the elements of $\{ \xi(x) \mid x \in \supp \kappa\} \subseteq G$ following the order $\leq$ and repeating $\kappa_\bz(x)$ times each element $\xi(x)$. 
    We will call $\gamma$ the \emph{$k$-tuple realizing $\kappa$ governed by $\xi$ and $\leq$}.
\end{defi}
