...
\begin{proof}
    ...
    $\barr \mu ( \bar a \bar b) = \barr\beta (\bar a, \bar b)$, for all $\bar a\in \barr A$ and $\bar b\in \barr B$, we have that $\mu (ab) = \barr \mu (ab) = \barr\beta (\bar a, \bar b) = \beta(a,b)$, for all $a\in A$ and $b\in B$. 
\end{proof}
...
\begin{proof}
    ...
    In view of \cref{lemma:barr-D-to-mc-M}, the choices of $\barr \D$ and $K$ give us a standard realization $(\D, \vphi_0)$ as in \ref{def:std-realization-MxM-QxQ}(a).
    Recall that a standard realization $\barr \D$ associated to $(\barr T, \bar \beta)$ is obtained by choosing subgroups $\barr A$ and $\barr B$ of $\barr T$ such that $\barr T = \barr A \times \barr B$ and $\barr \beta (\barr A, \barr A) = \barr \beta (\barr B, \barr B) = 1$. 
...
\begin{proof}
    ...
    By \cref{lemma:barr-D-to-mc-M}, with $(T^+, \beta^+)$ playing the role of $(T, \beta)$, the choices of $\barr \D\even$ and $K$ give us the same information as the choice of $\mc M$ in \cref{def:std-realization-MxM-QxQ}\eqref{item:choose-mc-M}, and the map associated to the transposition on $\mc M$ is $\mu^+\restriction_K$. 
    ...
    Since $\chi\inv\restriction_K$ and $\mu\restriction_{\langle f \rangle}$ are trivial, it follows from \cref{eq:fix-eta-plus-undouble-Q} that $\eta\restriction_{T^+} = \eta^+$. 
    ...
    Also, $\rad \barr\beta = \langle \barr t_p \rangle$, so $\D\epsilon$ is isomorphic to a standard realization of type Q associated to $(\barr {T^+}, \barr{\beta^+}, \barr h)$ (see discussion preceding \cref{def:standard-realization-Q}), \ie, it is isomorphic to $\barr \D \coloneqq Q(1) \tensor \barr \D\even$ as in \cref{rmk:change-Q(1)-of-place}. 
    ...
    Then, it follows that the map $\barr\mu$ as defined in \cref{eq:defi-mu-undoubled} is the map associated to the queer supertranspose on $\barr \D$. 
    Indeed, we already showed that $\eta\restriction_{T^+} = \eta^+$, hence, by \cref{eq:fix-eta-plus-undouble-Q}, we have that $\barr\mu\restriction_{\barr{T^+}} = \barr{\mu^+}$. 
    ...
    In the definition $\Gamma_Q^{\mathrm{(II)}}(T^+, \beta^+, h, \kappa, g_0)$, we identified $M_k(\barr \D\even) = M_k(\FF) \tensor \barr \D\even$ with $S\even = M_{n+1}(\FF)$ via Kronecker product. 
    It is clear that these identifications send $\Lambda \tilde\Phi\even \epsilon$ to $\Theta\even$. 
    ...
    \]
    the $\barr G$-grading on $M_k(\barr \D)$ becomes the $\barr G$-grading we defined on $S = Q(1) \tensor S\even$, and $\Lambda\tilde\Phi\epsilon$ is sent to $I_2 \tensor \Theta\even = \Theta$. 
...
\begin{proof}
    ...
    Hence, the result follows from \cref{thm:QxQ-type-I} for Type I gradings and from
    \cref{cor:QxQ-reduced-to-MxM} for Type II gradings. 
\end{proof}
...
\begin{remark}
    A $G$-grading on the Lie superalgebra $\osp(n+1,n+1)$ (respectively, $P(n)$) gives rise to a $G \times \ZZ_2$-grading on $A(m,n)$ of Type II\textsubscript{osp} (respectively, II\textsubscript{P}). 
\end{remark}

Recall that. for \cref{defi:type-II-A-m-not-n}, we have fixed a subgroup $K \subseteq T^+$ such that $T^+ = K \times \langle f \rangle$. 
...
\input{chapters/05_Gradings-on-Lie-Superalgebras/04_Ann-odd}
