...
\begin{defi}\label{def:sesquilinear-form}
    ...
    \end{enumerate}
    The super-anti-automorphism $\vphi_0$ can be specified by stating that $B$ is \emph{sesquilinear with respect to $\vphi_0$} or that $B$ is \emph{$\vphi_0$-sesquilinear}. 
...
We start with a definition to have a consise description of the graded superalgebras with superinvolution we are working on:


\begin{defi}
    Let $\D$ be a finite dimensional graded-division superalgebra over an algebraically closed field $\FF$, $\Char \FF \neq 2$, $\U$ be a graded right $\D$-module of finite rank and $B$ be a nondegenerate homogeneous sesquilinear form on $\U$ such that $\overline{B} = \pm B$. 
    We define $\mathcal{M} (\D, \U, B)$ to be the (finite dimensional) graded superalgebra with superinvolution $(\End_\D, \vphi)$, where $\vphi$ is the superadjunction with respect to $B$. 
    We will refer to $(\D, \U, B)$ as the \emph{defining triple} of $\mathcal{M} (\D, \U, B)$. 
    If $\D$ is associated to $(T, \beta, p)$ (see Definition ??) and $(\U, B)$ has inertia $(\eta, \kappa, g_0, \delta)$ (see Definition ??), then we say that $(T, \beta, p, \eta, \kappa, g_0, \delta)$ are the parameters of $(\D, \U, B)$. 
\end{defi}

\begin{defi}\label{def:defining-object}
    Let $\D$ be a finite dimensional graded-division superalgebra over an algebraically closed field $\FF$, $\Char \FF \neq 2$, $\U$ be a graded right $\D$-module of finite rank and $B$ be a nondegenerate homogeneous sesquilinear form on $\U$ such that $\overline{B} = \pm B$. 
    We say that $(\D, \U, B)$ is the \emph{defining object} of the finite dimensional graded-simple superalgebra with superinvolution $(R, \vphi)$, where $R \coloneqq \End_\D (\U)$ and $\vphi$ is the superadjunction with respect to $B$. 
    If $\D$ is associated to $(T, \beta, p)$ (see Definition ??) and $(\U, B)$ has inertia $(\eta, \kappa, g_0, \delta)$ (see Definition ??), then we say that $(T, \beta, p, \eta, \kappa, g_0, \delta)$ is the \emph{parametrization} of $(\D, \U, B)$. 
\end{defi}
...
\begin{thm}\label{thm:iso-(R,vphi)-with-parameters}
    Let $(R, \vphi)$ and $(R', \vphi')$ be finite dimensional graded-simple superalgebras with superinvolution with defining objects $(\D, \U, B)$ and $(\D', \U', B')$, respectively, and let $(T, \beta, p, \eta, \kappa, g_0, \delta)$ and $(T', \beta', p', \eta', \kappa', g_0', \delta')$ be their corresponding parametrizations. 
    Then $(R, \vphi) \iso (R', \vphi')$ if, and only if, $T = T'$, $\beta = \beta'$, $p = p'$, and $(\eta, \kappa, g_0, \delta)$ and $(\eta', \kappa', g_0', \delta')$ lie in the same orbit of the $T\times G^\#$-action on $\mathsf{X}(T, \beta, p)$ given on Definition \ref{def:TxG-action}.
\end{thm}
