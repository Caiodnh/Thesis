...
As it is usually dine for multisets, we define $|\kappa| \coloneqq \sum_{x \in \supp \kappa} \kappa(x)$. 
...
We can use $\gamma$ to construct to define the graded $\D$-module $\U \coloneqq {}^{[g_1]}\D \oplus \cdots \oplus {}^{[g_k]}\D$, which is clearly a representative of the isomorphism class of graded $\D$-modules determined by $\kappa$. 
