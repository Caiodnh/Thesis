...
\begin{prop}\label{prop:self-dual-components}
    ...
    \begin{enumerate}[(i)]
        ...
        \item $J_{\kappa(x)} \coloneqq \begin{pmatrix}
        0 & I_{\kappa(x)/2}\\
        -I_{\kappa(x)/2} & 0
    \end{pmatrix} \tensor X_t$ if $\mu_{x} = -1$ (in this case, $\kappa(x)$ must be even).
    \end{enumerate}
\end{prop}
...
\begin{proof}
    ...
    For the ``moreover'' part, it follows from Lemmas ?? and ?? that $B_x$ is nondegenerate and $\overline{B_x} = \delta B_x$ if, and only if, $\tilde B_x$ is nondegenerate and $B_x (u,v) = \mu_x B_x(v, u)$, for all $u,v \in \U_{\xi(x)}$. 
