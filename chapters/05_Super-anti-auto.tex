...
\begin{defi}
    Let $\U$ and $\U'$ be graded right $\D$-modules, and let $B$ and $B'$ be $\vphi_0$-sesquilinear forms on $\U$ and $\U'$, respectively. 
...
\begin{defi}
    Let $\U$ and $\U'$ be graded right $\D$-modules, and let $B$ and $B'$ be $\FF$-bilinear maps on $\U$ and $\U'$, respectively, taking values on $\D$. 
    A \emph{isomorphism from $(\U, B)$ to $(\U', B')$} is an isomorphism of graded modules $\theta\from \U \to \U'$ such that $B'( \theta(u), \theta(v) ) = B(u, v)$, for all $u,v \in \U$.
\end{defi}
...
\begin{cor}\label{cor:iso-abstract-vphi}
    ...
    \] 
    such that $\psi(r) = \psi_1 \circ r \circ \psi_1\inv$, for all $r\in R$ and $u\in \U$.
\end{cor}

\begin{cor}\label{cor:iso-abstract-vphi}
    Let $R$, $R'$, $\psi\from R \to R'$ and $(g, \psi_0, \psi_1)$ be as in Theorem \ref{thm:iso-abstract}. 
    Let $\vphi$ and $\vphi'$ be degree preserving super-anti-automorphisms on $R$ and $R'$ determined by pairs $(\vphi_0, B)$ and $(\vphi_0', B')$, respectively. 
    Then $\psi\from (R, \vphi) \to (R', \vphi')$ is an isomorphism of graded superalgebras with super-anti-automorphism if, and only if, there is $0\neq d\in \D$ a homogeneous element such that
    \[
        \psi_1 \from (\U^{[g]}, dB) \to ( (\U')^{\psi_0}, \psi_0\inv \circ B ) 
    \] 
    is an isomorphism of graded supermodules with bilinear map.
\end{cor}

\begin{proof}
    Consider $\vphi'' \coloneqq \psi\inv \vphi' \psi$. 
    We have that $\psi$ is an isomorphism of graded superalgebras with super-anti-isomorphism if, and only if, $\vphi = \vphi'$. 
    
    Put $B'(u, v) = \coloneqq \psi_0\inv( B'( \psi_1(u), \psi_1(v) ) )$, for all $u, v \in \U$. 
    ...
    \begin{align*}
        xy
    \end{align*}
    Hence, by Theorem \ref{thm:vphi-iff-vphi0-and-B}, $\vphi = \vphi''$ if, and only if, there is $d \in \D$ such that.
\end{proof}
...
\begin{defi}
    ...
    \end{enumerate}
    The set of $(g_0, \delta, \eta)$-admissible maps will be denoted by $\mathsf{X}(g_0, \delta, \eta)$.
\end{defi}
