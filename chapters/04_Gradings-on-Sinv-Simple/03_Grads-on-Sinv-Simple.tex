...
\begin{defi}\label{def:model-grd-M(m-n-0)}
    ...
    Let $T \subseteq G$ be a finite $2$-elementary subgroup, let $\beta\from {T\times T} \to \FF^\times$ be a nondegenerate alternating bicharacter, let $g_0 = (h_0, p_0) \in G^\#$ and let $\kappa_\bz, \kappa_\bo \from G/T \to \ZZ_{\geq 0}$ be $g_0$-admissible maps such that $k_\bz \sqrt{|T|} = m$ and $k_\bo \sqrt{|T|} = n$, where $k_\bz \coloneqq |\kappa_\bz|$ and $k_\bo \coloneqq |\kappa_\bo|$. 
    ...
    \]
    for all $X \in M(m,n)= M(k_\bz, k_\bo)$, where $\Phi$ is the matrix representing $B$ with respect to the chosen basis (viewed as an element of $M(m,n)$ rather than $M_{k_\bz | k_\bo} (\D)$). 
\end{defi}
