...
\begin{defi}
    We will call $L$ endowed with the $G$-grading constructed above the \emph{undoubled model} or the \emph{undoubling} of $\Skew(R, \vphi)/Z(\Skew(R, \vphi))$. 
\end{defi}
...
Clearly, $\theta_0$ is a super-anti-automorphism on $\D$ associated to the map 
\[\lable{eq:defi-mu-undoubled}
    \mu \coloneqq \eta \chi.
\]
...
\end{equation}
so $\barr\mu$ is well-defined for every coset $\bar t \in \barr T$. 
...
To summarize, the undoubled model is determined by a grading on $S$ making it isomorphic to $E(\barr T, \barr \beta, \barr p, \kappa) \iso M_k(\barr \D)$, a map $\mu \from \barr T \to \FF^{\times}$ associated to a super-anti-automorphism $\theta_0$ on $\barr(\D)$ and a matrix $\Theta \in M_k(\barr \D)$. 
...
\begin{proof}
    ...
    In other words, define $\D\even \coloneqq (\D\even)^{[g_1]}\oplus \cdots \oplus (\D\even)^{[g_k]}$, where $(g_1, \ldots, g_k)$ is the $k$-tuple of elements in $G$ realizing $\kappa$ according to $\xi$ and $\leq$, and $B$ by $B(u_i, u_j) \coloneqq \Phi_{ij}$, where $\B \coloneqq \{ u_1, \ldots, u_k \}$ is the canonical graded $\D\even$-basis of $\U\even$ and $\Phi \in M_k(\D\even)$ is the matrix defined by \cref{eq:puting-the-blocks-of-Phi-together}. 
    We then define $\U\odd \coloneqq \U\even \tensor_{\D\even} \D$. 
    Note that $\B$ is an even graded $\D$-basis of $\U$, and let $B\from \U \times \U \to \D$ the unique $\vphi_0$-sesquilinear extension of $B\even$, \ie, $B$ is also represented by $\Phi$, seen as a matrix in $M_k(\D)$.  
    
    % Undoubling what we just constructed
    We will now follow \cref{ssec:undoubling} to get the undoubled model of $Q^{\mathrm{ex}}(T^+, \beta^+, h, \kappa, g_0)$ constructed with the choices above. 
    % $\D$ is what it should be
    Let $\epsilon \coloneqq \frac{1+\zeta}{2}\in \D$ be a central idempotent, where $\deg \zeta = f$, and consider $\D\epsilon$ as a $\barr G$-graded-division superalgebra. 
    By \cref{prop:lemma-for-undoubling-and-fine-gradings}, the $\D\epsilon$ has parameters $(\barr T, \barr \beta, \barr p)$. 
    ...
    From now on, we will identify $\D\epsilon$ with $\barr \D$. %@1
    
    We need to show that $\mu$ as defined in \cref{eq:defi-mu-undoubled} is the one associated to the queer supertranspose. 
    
    We will now describe the matrix $\Phi\epsilon$. 
