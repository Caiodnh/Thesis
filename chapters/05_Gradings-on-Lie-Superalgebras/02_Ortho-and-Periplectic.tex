\section[Gradings on simple Lie superalgebras of series \texorpdfstring{$B$, $C$, $D$ and $P$}{B, C, D and P}]{Gradings on simple Lie superalgebras of series $B$, $C$, $D$ and $P$}\label{sec:grds-osp-and-p}
...
Recall that, since $\D$ is even, both $\U\even$ and $\U\odd$ are $G$-graded right $\D$-supermodules. 
...
\begin{defi}\label{defi:Gamma_P}
    Let $n\geq 2$, 
    let $T \subseteq G$ be a finite $2$-elementary subgroup, let $\beta\from {T\times T} \to \FF^\times$ be a nondegenerate alternating bicharacter, let $h_0 \in G$ and let $\kappa_\bz\from G/T \to \ZZ_{\geq 0}$ be a map with finite support such that $k \sqrt{|T|} = n+1$, where $k \coloneqq |\kappa_\bz|$. 
    Choose a $k$-tuple $\gamma_\bz = (g_1, \ldots, g_k)$ of elements in $G$ realizing $\kappa_\bz$, and define $\gamma_\bo \coloneqq (h_0\inv g_1\inv, \ldots, h_0\inv g_k\inv)$. 
...
let $x_1 < \cdots < x_{\ell_\bz}$ be the elements of $\{ x \in \supp \kappa_\bz \mid x \leq g_0\inv x\inv \}$ and, similarly, let $y_1 < \ldots < y_{\ell_\bo}$ be the elements of $\{ y \in \supp \kappa_\bo \mid y \leq g_0\inv y\inv \}$. 
...
Recall that the Lie superalgebras $\osp(m|n)$ are separated into series $B$, $C$ and $D$: the ones for which $m$ is odd constitute series $B$, the ones for which $m = 2$ constitute series $C$ and the remaining ones constitute series $D$ (see \cref{defi:B-C-D}). 
