...
\begin{lemma}\label{lemma:K-before-t_1}
    Let $(T, \beta, p)$ be as in \cref{thm:tensor-prd-decomposition}, with $T^- \neq \emptyset$, and let $K \subseteq T^+$ be any subgroup such that $T^+ = (\rad \beta^+) \times K$. 
    Then the set $S \coloneqq \{ t_1 \in T^- \mid \beta(t_1, K) = 1 \}$ is a coset of $\rad \beta^+$. 
\end{lemma}

\begin{proof}
    Its is straightforward that $t_1, t_1' \in S$ \IFF $t_1\inv t_1' \in \rad \beta^+$. 
    Hence we only have to show that $S \neq \emptyset$. 
    If $t_p \in T^-$ is odd, then $t_p \in \rad \beta$, so $t_p \in S$. 
    If $t_p \in T^+$, let $\chi$ be the character on $T^+$ determined by $\chi(K) = \chi(f) = 1$ and $\chi(t_p) = -1$, and extend $\chi$ to a character on $T$. 
...
\begin{remark}\label{rmk:choose-K-before-t_1}
    In the proof of \cref{thm:tensor-prd-decomposition}(b), we choose the complement $K \subseteq T^+$ for $\rad \beta^+$ such that $\beta(t_1, K) = 1$ depending on the (arbitrary) element $t_1 \in T^-$. 
    We could proceed in the other direction: first choose an arbitrary complement $K$, and then find a element $t_1$ such that $\beta(t_1, K) = 1$. 
    Indeed, $K$ is a vector space over $\ZZ_2$ and, hence, $\beta\restriction_{K \times K}\from K \times K \to \pmone \iso \ZZ_2$ is a nondegenerate alternating bilinear form. 
    So there is a symplectic basis $\B' = \{ a_1, b_1, \ldots, a_k, b_k\}$ for $K$, \ie, for all $1\leq i \leq k$, $\beta(a_i, a_j) = \beta(b_i, b_j) = 1$, and $\beta(a_i, b_j) = -1$, if $i = j$, and $\beta(a_i, b_j) = 1$, if $i \neq j$. 
    Start with an arbitrary element $t_1 \in T^-$ and, for all $i \in \{1,\ldots,k\}$ change it, in order, to $b_i t_1$ if $\beta(t_1, a_i) = -1$ and to $a_i t_1$ if $\beta(t_1, b_i) = -1$. 
    It is clear that the resulting $t_1$ is in $T^-$ and $\beta(t_1,a_1) = \beta(t_1, b_i) = 1$. 
    Further, another element $t_1' \in T^-$ is such that $\beta(t_1', K) = 1$ \IFF $t_1' \in t_1 \rad \beta^+$. 
\end{remark}
...
\begin{defi}\label{def:std-realization-MxM-QxQ}
    ...
    \begin{enumerate}[(i)]
        ...
        \item an element $t_1 \in T^-$ such that $\beta(t_1, K) = 1$, if we are in case (b);
    ...
    \end{enumerate}
    A subgroup $K$ in item \eqref{item:K-can-be-orthogonal-to-t_1} exists because $T^+$ is an elementary $2$-group and, in case (b), the element $t_1$ in \eqref{item:choose-t_1-std-realization} exists by \cref{rmk:choose-K-before-t_1}. 
...
\begin{remark}\label{rmk:eta-for-MxM-or-QxQ}
    ...
    Also, a different choice $t_1'$ in \eqref{item:choose-t_1-std-realization} leads to the the same $\eta$ if $t_1' \in \{ t_1, t_p t_1\}$, and to a different $\eta$ if $t_1' \in \{ f t_1, t_p f t_1\}$. 
\end{remark}
...
\begin{remark}\label{rmk:choose-other-t_1}
    Different choice of t1, same kappa but $g_0$ goes to $t_pg_0$ in half of the cases, up to isomorphism... check
\end{remark}

Note that, as a ungraded superalgebra with superinvolution, $M^{\mathrm{ex}}(T, \beta, t_p, \kappa, g_0) \iso M(n,n)\times M(n,n)\sop$, where $n = |\kappa| \sqrt{|T|/8}$, and $Q^{\mathrm{ex}}(T, \beta, t_p, \kappa, g_0) \iso Q(n)\times Q(n)\sop$, where $n = |\kappa| \sqrt{|T|/4}$.
...
Also, $g_0 \in G$ and the $g_0$-admissibility condition involves only elements in $G$. 
...
\input{chapters/04_Gradings-on-Sinv-Simple/06_Ann-only-G}