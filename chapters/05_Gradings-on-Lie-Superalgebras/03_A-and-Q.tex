...
Let $\barr \D\even$ be a standard realization associated to $(\barr{T^+}, \barr{\beta^+})$ (see \cref{def:standard-realization}), and let $\barr {\mu^+}\from \barr{T^+} \to \FF^\times$ be the map associated to the transposition map. 
...
Since $\theta_0$ is the transposition on $\barr \D\even$, $\theta_0(X)\stransp = \theta_0(X)\transp \in M_{k} (\barr \D\even)$ becomes $X\stransp = X\transp \in M_{n+1}(\FF)$. 
Hence, \cref{eq:theta-with-matrix-2} reduces to 
\[\label{eq:theta-with-matrix-4}
    \forall X\in M_{n+1}(\FF), \quad \theta(X) = \Theta_\bz\inv\, X\transp\, \Theta_\bz.
\]
...
\[\label{eq:theta-with-matrix-5}
    \forall X\in Q(n+1), \quad \theta(X) = \Theta\inv\, X\stransp\, \Theta,
\]
where 
\[
...
If the grading on $(R,\vphi)$ is even, then, by \cref{thm:MxM-even}, $R$ is isomorphic to $M^{\mathrm{ex}} (T, \beta, \kappa_\bz, \kappa_\bo, g_0)$, where $T$ is a finite elementary $2$-group, $\beta\from {T\times T} \to \FF^\times$ is an alternating bicharacter with $\rad \beta = \langle f \rangle$ for some $f \in T$, $g_0$ is an element in $G^\#$, and $\kappa_\bz, \kappa_\bo \from G/T \to \ZZ_{\geq 0}$ are $g_0$-admissible maps (see \cref{inertia-even-and-odd-case}) such that $|\kappa_\bz| \sqrt{|T|} = |\kappa_\bo| \sqrt{|T|} = n+1$ (see \cref{def:model-grd-MxM-even}). 
