...
\begin{proof}
    ...
    Then the result follows from a well-known fact from linear algebra over algebraically closed fields and that an $\FF$-basis for $\U_{\xi(x)}$ is a $\D$-basis for $\U_x$.
\end{proof}
...
\begin{defi}
    ...
    \begin{enumerate}[(i)]
        ...
        \item $\kappa(x) = \kappa(g_0\inv x)$ for all $x \in G^\#/T$; \label{item:kappa-duality}
