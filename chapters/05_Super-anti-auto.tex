...
If $g\in G^\#$ is odd, a $\vphi_0$-sesquilinear form $B$ on the $\D$-module $\U$ is not $\vphi_0$-sesquilinear if considered on the $\D$-module $\U^{[g]}$, and Equation \eqref{eq:superadjunction} do not determine the same super-anti-automorphism $\vphi$. 
...
% For that, we will have an adaptation of Theorem \ref{thm:iso-abstract}.
...
\begin{cor}\label{cor:iso-abstract-vphi}
    ...
    \[
        \psi_1 \from (\U^{[g]}, dB) \to ( (\U')^{\psi_0}, \psi_0\inv \circ B' ) 
    \] 
...
\begin{proof}
    ...
    Let us define $\vphi'' \coloneqq \psi\inv \circ \vphi' \circ \psi$. 
    Then $\psi$ is an isomorphism of graded superalgebras with super-anti-isomorphism if, and only if, $\vphi = \vphi''$. 
    Consider $B''(u, v) \coloneqq \psi_0\inv( B'( \psi_1(u), \psi_1(v) ) )$, for all $u, v \in \U$. 
    We claim that $B''$ is $\psi_0\inv \vphi_0' \psi_0$-sesquilinear. 
    Indeed:
    \begin{align*}
        B''(u, vd) &= \psi_0\inv( B'( \psi_1(u), \psi_1(vd) ) ) 
        = \psi_0\inv( B'( \psi_1(u), \psi_1(v) ) \psi_0(d) ) = B''(u, v)d
        \intertext{and}
        B''(ud, v) &= \psi_0\inv( B'( \psi_1(ud), \psi_1(v) ) ) = \psi_0\inv( B'( \psi_1(u) \psi_0(d), \psi_1(v) ) )\\
        &= \psi_0\inv( (-1)^{(|B''| + |u|) |d|} \vphi_0(\psi_0(d)) B'( \psi_1(u), \psi_1(v) ) ) \\
        &= (-1)^{(|B''| + |u|) |d|} \psi_0\inv( \vphi_0( \psi_0 (d) ) ) B''(u, v).
    \end{align*}
    Also, $\vphi''$ is the superadjunction with respect to $B$:
    \begin{align*}
        B''(ru, v) &= \psi_0\inv( B'( \psi_1(ru), \psi_1(v) ) ) 
        = \psi_0\inv( B'( (\psi_1 \circ r \circ \psi_1\inv) \psi_1(u), \psi_1(v) ) ) \\
        &= \psi_0\inv( B'( \psi(r) \psi_1(u), \psi_1(v) ) ) \\
        &= \psi_0\inv( \sign{\psi(r)}{\psi_1(u)} B'( \psi_1(u), \vphi'( \psi(r) ) \psi_1(v) ) ) \\
        &= \sign{r}{u} \psi_0\inv( B'( \psi_1(u), (\vphi'\circ \psi)(r) ) \psi_1(v) ) )\\
        &= \sign{r}{u} \psi_0\inv( B'( \psi_1(u), (\psi\circ \vphi'')(r) ) \psi_1(v) ) )\\
        &= 
    \end{align*}
    Hence, by Theorem \ref{thm:vphi-iff-vphi0-and-B}, $\vphi = \vphi''$ if, and only if, there is $0 \neq d \in \D$ such that $B'' = dB$. 
    The result follows. 
\end{proof}
