...
In this section, we will recall the classification of gradings on matrix algebras \cite{BSZ01, BZ02, BK10} and, more generally, that of graded-simple algebras satisfying the \dcc on one-sided graded ideals (see below). 
We will follow the exposition of \cite[Chapter 2]{livromicha} but use a slightly different notation for parameters, which will be extended to superalgebras in \cref{sec:grd-simple-salg}.
...
\end{proof}
    ...
    % To show that $e \in T$, we will prove that $1 \in \D_e$. 
    % Let us write $1 = \sum_{g\in G} a_g$, where $a_g \in \D_g$. 
    % For any $h\in G$, $a_h = a_h 1 = \sum_{g\in G} a_h a_g \in D_h$ and, since $a_h a_g \in \D_{hg}$, the only nonzero element in this sum $a_h a_e$. 
    % It follows that $a_h = a_h a_e$ and, hence, $1 = \sum_{g\in G} a_g a_e = 1 a_e = a_e \in \D_e$. 
    
    % Finally, take $t \in T$ and $0\neq X_t \in \D_t$ and write $X_t\inv =  \sum_{g\in G} b_g$. 
    % Then $1 = X_t X_t\inv = \sum_{g\in G} X_t b_g$ and $1 = X_t\inv X_t = \sum_{g\in G} b_g X_t$. 
    % Since $1 \in \D_e$, it follows that $1 = X_t b_{t\inv}$ and $1 = b_{t\inv} X_t$, so $X_t\inv = b_{t\inv} \in \D_{t\inv}$. 
...
Since $\D_e$ is an (ungraded) division algebra, every right $\D_e$-module has a $\D_e$-basis. 
...
\begin{defi}\label{defi:tuple-governed}
...
The next question is: when are two graded algebras $\End_\D (\U)$ and $\End_{\D'} (\U')$ isomorphic? 
...
Thus, $\U$ is a graded $(R, \D)$-bimodule.
...
The following result is essentially \cite[Exercise 3 on page 60]{livromicha}.
...
\begin{lemma}\label{lemma:converse-density-thm}
    As a left $R$-module, $\U$ is graded-simple. 
    Also, the right action of $\D$ on $\U$ gives an isomorphism $\rho\from \D \to \End_R(\U)$. 
\end{lemma}
...
\begin{proof}
    ...
    Let $\{u_1, \ldots, u_k \}$ be a graded $\D$-basis of $\U$. 
    ...
    \]
    Therefore $f = \rho(d_1)$, proving that the image of $\rho$ is the whole $\End_R (\U)$. 
    
    Finally, since any nonzero homogeneous element $u_1 \in \U$ can be included in a graded $\D$-basis $\{u_1, \ldots, u_k \}$, the maps $r$ defined as above show that $\U = R u_1$ and, hence, $\U$ is graded-simple as an $R$-module. 
\end{proof}

Recall that, if $G$ is abelian, the center of a graded algebra is a graded subalgebra (\cref{lemma:center-is-graded}). 
...
\begin{cor}[{\cite[Theorem 2.6]{BK10},\cite[Theorem 2.27]{livromicha}}]\label{cor:grds-matrix-alg}
...
As an application of \cref{thm:End-over-D,thm:iso-End_D-U-with-parameters}, we can obtain the classification of finite dimensional simple superalgebras over an algebraically closed field. 

\begin{thm}[\cite{MR167498}]\label{thm:fd-simple-SA}
    Let $R$ be a finite dimensional simple superalgebra over an algebraically closed field $\FF$. 
    Then $R$ is isomorphic to either $M(m,n)$ or $Q(n)$, where $m,n \in \ZZ_{\geq 0}$. 
    Moreover,
    \begin{enumerate}[(i)]
        \item $M(m,n) \not\iso Q(n')$;
...
\begin{proof}
    ...
    Finally, using the Kronecker product, we get $M_n(\FF) \tensor Q(1) \iso Q(1)\tensor M_n(\FF) \iso Q(n)$. 
\end{proof}
