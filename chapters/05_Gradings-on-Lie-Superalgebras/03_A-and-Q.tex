...
THE NEXT REMARK IS NOT CAREFULLY WRITTEN. 
WHAT DO YOU THINK ABOUT ITS EXISTENCE?
...
\begin{defi}\label{defi:type-II-Q}
    ...
    \end{enumerate}
    Let $\barr {\mu^+}\from \barr T^+ \to \FF^\times$ be the map associated to the transposition on $\barr \D\even$ (see \cref{ssec:param-D-vphi}). 
    Let $\chi \in \widehat{T}$ be the character such that $\chi(K) = 1$ and $\chi(f) = -1$, and extend it to a character on $\widehat{G}$, also denoted by $\chi$. 
    Then define $\mu \coloneqq \barr {\mu^+} \circ \pi$, where $\pi\from G \to \barr G$ is the natural homomorphism, and define $\eta^+\from T^+ \to \pmone$ by \cref{eq:fix-eta-plus-undouble-Q}.
    ...
    Then construct a tuple $\bar\gamma$ realizing $\kappa$ according to $\pi \circ \xi$ and $\leq$ (\cref{defi:tuple-governed}) and
    consider the corresponding elementary $\barr G$-grading on $M_k(\FF)$, where $k \coloneqq |\kappa|$. 
    Then consider the (division) $\barr G$-grading on the associative superalgebra $Q(1)$ given by setting $\deg u \coloneqq \bar h$, where %@1
    $
        u \coloneqq
        \left(\begin{array}{c|c}
            0 & 1\\
            ...
            1 & 0\\
        \end{array}\right)
    $, and the $\barr G$-grading on the associative superalgebra $S \coloneqq Q(n+1)$ by identifying it with $Q(1)\tensor M_k(\FF) \tensor \barr \D\even$ via Kronecker product. 
    We define $\theta\from S \to S$ to be the map in \cref{eq:theta-with-matrix-5}, where $\Theta$ and $\Theta_\bz$ are defined by \cref{eq:Theta-for-Q,eq:Theta_bz-for-Q}, respectively. 
    Finally, we define $\Gamma_Q^{\mathrm{(II)}}(T^+, \beta^+, h, \kappa, g_0)$ to be the $G$-grading on $L = S^{(1)}/Z(S^{(1)})$ induced from the $G$-grading $S^{(1)} = \bigoplus_{g\in G} S^{(1)}_g$, where
    \[
        S^{(1)}_{g} \coloneqq \{ s\in S^{(1)}_{\bar g} \mid \theta (s) = - \chi(g) s \},
    \]
    for all $g\in G$. 
