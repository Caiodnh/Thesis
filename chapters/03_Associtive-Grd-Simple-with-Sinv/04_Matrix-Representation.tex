...
% \begin{proof}
%     First of all, note that Equation \ref{eq:superadjunction} is equivalent to, for all $u_i, u_j \in \mc B$,
%     %
%     \[
%         B(ru_i, u_j) = \sign{r}{i} B(u_i, \vphi(r) u_j).
%     \]
%     By the definitions of $X$, $Y$ and $\Phi$, we have that
%     %
%     \begin{align*}
%         B\bigg(\sum_{\ell=1}^k u_\ell x_{\ell i}, u_j\bigg) &= \sum_{\ell=1}^k (-1)^{( |B| + |\ell|) |x_{\ell i}|} \vphi_0( x_{\ell i} ) B (u_\ell x_{\ell i}, u_j)\\ &= \sum_{\ell=1}^k (-1)^{( |B| + |\ell|) |x_{\ell i}|} \vphi_0( x_{\ell i} ) \Phi_{\ell j}
%         \intertext{and}
%         \sign{r}{i} B\bigg(u_i, \sum_{\ell=1}^k u_\ell y_{\ell j}\bigg) 
%         &= \sign{r}{i} \sum_{\ell=1}^k B(u_i, u_\ell) y_{\ell j}\\
...
%     Let $p,q \in \{1, \ldots, k\}$, and suppose $X$ is such that $x_{pq}$ is a nonzero $G^\#$-homogeneous element of $\D$ and $x_{ij} = 0$ elsewhere, \ie, $X$ represents the map $r \in \End_\D(\U)$ defined by $r u_i = \delta_{iq} u_p x_{pq}$. 
...
%     Note that $|r| = |u_q| + |x_{pq}| - |u_p| = |p| + |q| + |x_{pq}|$. 
%     Then
%     %
%     \begin{align*}
%         B\bigg(\sum_{\ell=1}^k u_\ell x_{\ell i}, u_j\bigg) = B (u_p x_{pi}, u_j) &= (-1)^{ (|B| + |p|) |x_{pi}|} \vphi_0(x_{pi}) B(u_p, u_j)
