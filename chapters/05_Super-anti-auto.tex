...
\begin{lemma}\label{lemma:self-dual-components}
    Let $\U_{gT}$ be a self-dual isotypic component of $\U$, let $0 \neq X_t \in \D_t$, where $t \coloneqq g_0g^2$, and set $k \coloneqq \kappa(gT)$. 
    % set 
...
For that, we set $k \coloneqq |\kappa|$ and choose a tuple $\gamma = (g_1, \ldots, g_\ell, \ldots g_k) \in (G^\#)^{k}$ such that:
\begin{enumerate}[(i)]
    \item 
\end{enumerate}

We say that a put $|\kappa|$
