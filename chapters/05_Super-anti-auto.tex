...
Recall the concept of twist of a module (Definition \ref{def:twist}).
...
We will define 3 left group actions on the class of pairs $(\U, B)$, where $\U \neq 0$ is a graded $\D$-supermodule and $B$ is a nondegenerate homogeneous $\vphi_0$-sesquilinear form, for some $\vphi_0$ super-anti-automorphism on $\D$. 
Recall that, since $\U \neq 0$ and $B$ is nondegenerate, $\vphi_0$ is determined by $B$ (see ??).

Let $\D^\times_{\mathrm{gr}} \coloneqq \{ 0\neq d \in \D \mid d \text{ is homogeneous }\}$. 
Clearly, $\D^\times_{\mathrm{gr}}$ is a group with the multiplication on $\D$. 
Given $d\in \D^\times_{\mathrm{gr}}$, we define 
\begin{equation}\label{eq:Dx_gr-action}
    d\cdot (\U, B) \coloneqq (\U, dB).
\end{equation}
Note that $dB$ is $\vphi_0 \circ \mathrm{sInt}_d$-sesquilinear.
...
\end{equation}
Note that $\psi_0 \circ B$ is $(\psi_0 \circ \vphi_0 \circ \psi_0\inv)$-sesquilinear.
...
\end{equation}
Note that $B^{[g]}$ is $\vphi_0$-sesquilinear.

\begin{lemma}\label{lemma:action-on-(U,B)}
    The 3 actions defined above give rise to a $(\D^\times_{\mathrm{gr}} \rtimes A) \times G^\#$-action, where $A$ acts on $\D^\times_{\mathrm{gr}}$ by evaluation. 
\end{lemma}

\begin{proof}
    Let $d\in \D^\times_{\mathrm{gr}}$, $\psi_0 \in A$, $g \in G^\#$ and $u, v \in \U$. 
    First note that the action of $d$ does not change $\U$, hence commutes in the first entry with the other actions. 
    But 
    \begin{align*}
        (\psi_0 \circ dB)(u,v) &= \psi_0 \big( (d B(u,v) \big) = \psi_0 (d) \psi_0 \big( B(u,v) \big) = \big( \psi_0(d) (\psi_0 \circ B) \big) (u,v),
    \end{align*}
    and, hence, the $\D^\times_{\mathrm{gr}}$-action combined with the $A$-action gives us a $\D^\times_{\mathrm{gr}} \rtimes A$-action. 
...
\begin{cor}\label{cor:iso-with-actions}
    ...
    Then $(R, \vphi) \iso (R', \vphi')$ if, and only if, $((\U')^{\psi_0}, \psi_0\inv \circ B')$ is isomorphic to a pair in the $(\D^\times_{\mathrm{gr}} \rtimes A) \times G^\#$-orbit of $(\U, B)$. 
...
\section{Classification of graded-simple superalgbras with superinvolution}
...
\begin{defi}\label{def:parameter-of-(U,B)}
    ...
    We say that $(g_0, \delta, \kappa)$ are the \emph{parameters assigned to $(\U, B)$}, where $g_0 = \deg B \in G^\#$ and $\kappa(x) = \dim_\D \U_x$ for all $x \in G^\#/T$. 
\end{defi}
...
\begin{defi}
    ...
    We say that the triple $(g_0, \delta, \kappa)$ is \emph{$\vphi_0$-admissible} or \emph{$\eta$-admissible}if:
    \begin{enumerate}[(i)]
...
Given a triple $(g_0, \delta, \kappa) \in \mathsf{X} (\D, \vphi_0)$, we can construct a pair $(\U, B)$ such that $(g_0, \delta, \kappa)$ are assigned to it. 
...
\begin{proof}
    ...
    Conversely, suppose that $(\U, B)$ and $(\U, B')$ have the same parameters $(g_0, \delta, \kappa)$. 
...
From Theorem \ref{thm:vphi-iff-vphi0-and-B}, we know that a graded-simple superalgebra is described by $\D$, $\vphi_0$, $\U$ and $B$. 
With the parametrizations we presented in this section,
in the case $\vphi$ is a superinvolution, $\D$ is finite dimensional and $\FF$ is algebraically closed,
this data reduces (up to isomorphism) to $T$, $\beta$, $p$, $\eta$, $g_0$, $\delta$ and $\kappa$. 
We will now classify the graded superalgebras with superinvolution in terms of this parameters.

To do that, we will follow Corollary \ref{cor:iso-with-actions} and the group actions defined by Equations \eqref{eq:Dx_gr-action}, \eqref{eq:Aut(D)-action} and \eqref{eq:G-action}. 
If $\Gamma(T, \beta, p, \eta, g_0, \delta, \kappa) \iso \Gamma(T', \beta', p', \eta', g_0', \delta', \kappa')$, then $T = T'$, $\beta = \beta'$ and $p = p'$, \ie, $\D \iso \D'$. 
Hence, let us consider these parameters fixed and focus on quadruples $(\eta, g_0, \delta, \kappa)$, where $(g_0, \delta, \kappa)$ is $\eta$-admissible. 

First, in our case, the $A$-action is trivial. 
To see that,
fix $\eta$ and suppose $(g_0, \delta, \kappa)$ is assigned to $(\U, B)$. 
Note that $\psi_0 \in A$ must be a scalar operator on each component $\D_t$ of $\D$, hence $\psi_0$ commutes with $\vphi_0$. 
It follows that $\psi_o\inv \circ B$ is $\vphi_0$-sesquilinear, so we can use Theorem ?? to check if $(\U, B)$ is isomorphic to $\psi_0\cdot (\U, B) = (\U^{\psi_0\inv}, \psi \circ B)$. 
The graded $\D$-modules $\U$ and $\U^{\psi_0\inv}$ have the same $\kappa$, since $\dim_\D \U_x = \dim_\D (\U_x)^{\psi_0\inv}$. 
...
The action defined by Lemma \ref{lemma:action-on-(U,B)} can, hence, be considered only a $\D^\times_{\mathrm{gr}} \times G^\#$-action. 
We will do another simplification now. 
Since $\FF^\times \subseteq \D^\times_{\mathrm{gr}}$ acts trivially, then we have an action by $\D^\times_{\mathrm{gr}} / \FF^\times \iso T$. 
Hence we get only a $T \times G^\#$-action. 

This action is given by:
\begin{align}
    t \cdot (\eta, g_0, \delta, \kappa) &= (\eta \tilde \beta (t, \cdot), t g_0, (-1)^{|t|} \eta(t)\delta, \kappa)
    \intertext{and}
    g \cdot (\eta, g_0, \delta, \kappa) &= (\eta, g_0 g^{-2}, (-1)^{|g|} \delta, g\cdot \kappa),
\end{align}
where $(g \cdot \kappa)(x) \coloneqq \kappa (g\inv x)$ for all $x \in G^\#/T$.

We are now in position to give a classification up to isomorphism 

Section \ref{sec:iso-vphi-abstract}, Lemma \ref{lemma:action-on-(U,B)}, Equations \eqref{eq:Dx_gr-action}, \eqref{eq:Aut(D)-action} and \eqref{eq:G-action}.