...
First, in our case of algebraically closed $\FF$, the $A$-action is trivial. 
To see that,
...
Hence, we consider only the action by $\D^\times_{\mathrm{gr}} \times G^\#$. 

{\tt Explain:}
Again, since $\FF$ is algebraically closed, $\FF^\times \subseteq \D^\times_{\mathrm{gr}}$ acts trivially, so we actually have an action by $\D^\times_{\mathrm{gr}} / \FF^\times \iso T$. 
