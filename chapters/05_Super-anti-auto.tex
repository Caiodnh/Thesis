...
\begin{ex}\label{ex:main-extension}
    Consider $E \coloneqq \big(\pmone \times T^+\big) \cup \big(\{\pm i\} \times T^-\big)$ as a subgroup of $\{ \pm 1, \pm i \} \times T$, and let $\iota\from \pmone \to E$ and $\pi\from E \to T$ be the homomorphisms given by $\iota(\lambda) = (\lambda, e)$ and $\pi (\lambda, t) = t$ for all $\lambda \in \pmone$ and $t\in T$. 
...
\begin{proof}
    ...
    We can consider the $2$-cocycle $\sigma$ as admitting values in $\{\pm 1, \pm i\}$ and, in this case, . 
    In this case, we have that $\pmone \times_\sigma T \subseteq \{\pm 1, \pm i\} \times_\sigma T$. 
\end{proof}
...
\begin{thm}\label{thm:refinement-DxDsop}
    ...
    Consider $\mc E \coloneqq \D \times \D\sop$ with its natural $T$-grading $\Gamma\colon \mc E = \bigoplus_{t\in T} \D_t \times \overline{\D_t}$ and let $\vphi$ be the exchange superinvolution on $\mc E$. 
    There is a division grading on $(\mc E, \vphi)$ refining $\Gamma$ if, and only if, $\beta$ takes values in $\{ \pm 1 \}$. 
    Further, if $\Delta$ is such a refinement, then:% and $\widetilde T$ is its support, then:
...
\begin{proof}
    First of all, note that, for every $t\in T$, the homogeneous component $\mc E_t$ is $2$-dimensional and $\vphi$-invariant. 
    Moreover, the eigenspaces of $\vphi \restriction_{\mc E_t}$ associated to $1$ and $-1$ are both one dimensional, spanned by $(X_t, \overline{X_t})$ and $(X_t, -\overline{X_t})$, respectively. 
    ...
    We will denote the subspace generated by $(X_t, \lambda\overline{X_t})$, for $\lambda \in \{ \pm 1 \}$, by $\mc E_{(\lambda, t)}$. 
    Clearly, $\mc E$ is the direct sum of all subspaces of this form. 
    
    Now, for all $a, b \in T$ and $\lambda_1, \lambda_2 \in \{ \pm 1 \}$, we have:
    \begin{align*}
        (X_{a}, \lambda_1 \overline{X_{a}})(X_{b}, \lambda_2 \overline{X_{b}}) &= (X_{a} X_{b}, \lambda_1 \lambda_2 \overline{X_{a}} \,\overline{X_{b}}) \\
        &= (X_{a} X_{b}, \lambda_1 \lambda_2 (-1)^{p(a)p(b)} \overline{X_{b}X_{a}})\\ 
        &=(X_{a} X_{b}, \lambda_1 \lambda_2 (-1)^{p(a)p(b)} \beta(a, b) \overline{X_{a} X_{b}})\\
        &= (X_{a} X_{b}, \lambda_1 \lambda_2 \tilde\beta (a,b) \overline{X_{a} X_{b}}).
    \end{align*}
    %
    Clearly, if $\beta(a, b) \neq \pm 1$ for some $a,b \in T$, then this direct sum decomposition do not correspond to a grading. 
    On the other hand, if $\beta$ takes values in $\{ \pm 1 \}$, it follows that $\mc E_{(\lambda_1, a)} \mc E_{(\lambda_2, b)} \subseteq \mc E_{(\lambda_1 \lambda_2 \tilde\beta (a,b), ab)}$. 
    ...
    Now let $\Delta$ be any grading on $(\mc E, \vphi)$ refining $\Gamma$ and consider $E \coloneqq \supp \Delta$. 
    Recall that a division grading is a fine grading with its support being its universal group (Lemma ??). 
    Hence there is a unique map $\tilde\pi\from \widetilde T \to T$ such that ${}^{\pi}\widetilde \Gamma = \Gamma$. 
    ...
    Now let $\Delta'$ be another such refinement and let . 
    Since the gradings $\Delta$ and $\Delta'$ must have the same components, $\Delta$ is a (nonproper) refinement of $\Delta'$, so there is a unique group homomorphism $\alpha\from \widetilde T \to \pmone \times_{\tilde\beta} T$ such that ${}^\alpha \Delta = \Delta'$. 
    ...
    We conclude not only that the extensions $\pmone \xrightarrow{\iota} \widetilde T \xrightarrow{\pi} T$ and $\pmone \xrightarrow{\iota'} \pmone \times_{\tilde\beta} T \xrightarrow{\pi'} T$ are equivalent (and, hence, \eqref{item:equivalent-extension-example}), but also that $\alpha$ is the unique equivalence between them such that ${}^\alpha \Delta = \Delta'$. 
    ...
    For each $\tilde t\in \widetilde T$, there are unique $\lambda \in \pmone$ and $t\in T$ such that the component of $\tilde t$ with respect to $\widetilde \Gamma$ is $\mc E_{(\lambda, t)}$. 
    Let us define 
    
    Let $\pi\from \widetilde T \to T$ be defined by $\pi(\tilde t) = t$ and $\alpha\from \widetilde T \to \pmone \times_{\tilde\beta} T$  be defined by $\alpha(\tilde t) = (\lambda, t)$. 
    ...
    Also, $\alpha$ is necessarily a bijection, since every component $\mc E_{(\lambda, t)}$ also corresponds to a unique component of the grading $\widetilde \Gamma$. 
    ...
    % From this and the assumption that $\beta$ takes values in $\{ \pm 1 \}$, it follows that we can grade $\mc E$ by the group $\{ \pm 1\} \times_{\tilde\beta} T$ by defining the degree of $(X_t, \lambda\overline{X_t})$ to be $(\lambda, t)$, for all $t \in T$ and $\lambda \in \{ \pm 1 \}$.
...
Following the construction on the proof of Theorem \ref{thm:refinement-DxDsop}, we get a $\pmone \times_{\tilde\beta} T$-grading on $\D\times \D\sop$ such that $\deg (X_t, \lambda \overline{X_t}) = (\lambda, t)$, for all $\lambda \in \pmone$ and $t\in T$. 
...
We then get a grading by $\pmone \times_\sigma T$, where $\sigma(a,b) \coloneqq (-1)^{p(a)p(b)}$, where $\deg (X_t, \lambda \overline{X_t}) = (\mu(t) \lambda, t)$, for all $\lambda \in \pmone$ and $t\in T$. 
...
\begin{itemize}
    ...
    \item we can describe then using automorphisms od $\mc O$
\end{itemize}
