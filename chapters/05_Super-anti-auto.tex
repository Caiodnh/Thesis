...
If $x = y$, we say that $\U_x$ is a \emph{self-dual component} and, if $x\neq y$, we say that $\U_x$ and $\U_y$ are a \emph{pair of dual components}. 
...
\begin{proof}
    ...
    Then $B_x(u, v) = X_{t} \tilde{B}_x(u,v) = 0$ for all $v \in V_x$ and, hence, $B_x(u, vd) = B_x(u, v)d = 0$ for all $v \in \V_x$. 
    ...
    Then $B_x(\sum_{g\in G^\#} u_g, v) = \sum_{g\in G^\#} B_x(u_g, v) = 0$ and, since the summands have pairwise distinct degrees, $B_x(u_g, v) = 0$ for all $g\in G^\#$. 
    Also, since $g \xi(gT)\inv \in T$, we have that $u_g = \tilde u_g d$ for some homogeneous elements $\tilde u_g \in V_x$ and $0 \neq d \in \D$. 
    Then $0 = B_x(u_g, v) = (-1)^{|d|(|B| + |\tilde u_g|)} \vphi_0(d) B_x(\tilde u_g, v)$, and hence $B_x(\tilde u_g, v) = 0$. 
    We conclude that $\tilde{B}_x(\tilde u_g, v) = 0$, for all $v\in V_x$, which implies $\tilde u_g = 0$. 
...
\begin{proof}
    ...
    $
        \sign{u}{v} \tilde{B}_x(v, u) \eta(t)\inv X_t = \delta \tilde{B}_x(u, v) X_t
    $,
