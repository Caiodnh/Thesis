...
\[\label{eq:defi-mu-undoubled}
    \forall \bar t \in \barr T, \barr\mu \coloneqq \eta(t) \chi(t)
\] is well-defined. 
...
\begin{proof}
    ...
    We need to extend $\chi$ to $\widehat{G^\#}$. 
    ...
    We already showed that $\eta\restriction_{T^+} = \eta^+$, hence, by \cref{eq:I-eta-plus}, we have that $\barr\mu\restriction_{T^+}$, indeed, is the transposition on $\barr \D\even$. 
