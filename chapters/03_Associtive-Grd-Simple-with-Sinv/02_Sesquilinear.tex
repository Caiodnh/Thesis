...
and, in this way, $\mc U\Star$ becomes an $(R\sop, \D\sop)$-superbimodule.
...
\begin{proof}
	By definition of the superdual of an operator, the left $R\sop$-action on $\U\Star$ corresponds to the representation $R\sop \to \End_{\D\sop} (\mc U\Star)$ given by $r \mapsto r\Star$, which is an isomorphism since $\U$ is a graded $\D$-supermodule of finite rank (\cref{prop:dual-super-anti-iso}). 
	Identifying $R\sop$ and $\End_{\D\sop} (\mc U\Star)$ via this isomorphism, we can apply
