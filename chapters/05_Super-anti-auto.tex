...
\begin{proof}
    ...
    Consider $\vphi_0'' \coloneqq \psi_0\inv \circ \vphi_0' \circ \psi_0$ and $B''\from \U^{[g]} \times \U^{[g]} \to \D$ defined by $B''(u, v) \coloneqq \sign{u}{g}\psi_0\inv ( B'( \psi_1(u), \psi_1(v) ) )$, for all $u, v \in \U$. 
    ...
    \begin{align*}
        ...
        &= (-1)^{(|B''| + |u|) |d|} \vphi_0 \big( \psi_0 (d) \big) B''(u, v).
    ...
    \end{align*}
    Hence, applying Theorem \ref{thm:vphi-iff-vphi0-and-B} for $\End_\D (\U^{[g]})$ and Lemma \ref{lemma:B^[b]-does-the-job}, we conclude that $\vphi = \vphi''$ if, and only if, there is $0 \neq d \in \D$ such that $B'' = dB^{[g]}$. 
    The result follows. 
\end{proof}
