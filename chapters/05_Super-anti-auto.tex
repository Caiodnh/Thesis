...
\begin{defi}\label{def:coboundary}
    ...
    We define $\mathrm{d} f\from H\times H \to K$ to be map given by $\mathrm{d} f\, (a,b) = f(ab)f(a)\inv f(b)\inv$ for all $a,b \in H$. 
...
We state and proof the following for further reference.
...
\begin{proof}
    ...
    $\tilde\beta (t, T) = 0$ if, and only if, $\beta (t, T) = 0$. 
...
\begin{defi}\label{def:parameter-of-(U,B)}
    ...
    We say that $(\kappa, g_0, \delta)$ are the \emph{parameters assigned to $(\U, B)$}, where $g_0 = \deg B \in G^\#$ and $\kappa(x) = \dim_\D \U_x$ for all $x \in G^\#/T$. 
\end{defi}

By the considerations above, the triple $(\kappa, g_0, \delta)$ satifies the following: 

\begin{defi}
    Let $\kappa\from G^\#/T \to \ZZ_{\geq 0}$, $g_0 \in G^\#$ and $\delta \in \pmone$. 
    We say that the triple $(\kappa, g_0, \delta)$ is \emph{$\vphi_0$-admissible} or \emph{$\eta$-admissible} if:
    \begin{enumerate}[(i)]
    ...
    \end{enumerate}
    The set of all $\vphi_0$-admissible triples will be denoted by $\mathsf{X}(\D, \vphi_0)$.
\end{defi}

Given a triple $(\kappa, g_0, \delta) \in \mathsf{X} (\D, \vphi_0)$, we can construct a pair $(\U, B)$ such that $(\kappa, g_0, \delta)$ are assigned to it. 
...
\begin{proof}
    ...
    Conversely, suppose that $(\U, B)$ and $(\U, B')$ have the same parameters $(\kappa, g_0, \delta)$. 
...
For each triple $(T, \beta, p)$, we fix a graded-division superalgebra $\D$ with these parameters, so we may suppose $\D \iso \D'$. 

\begin{defi}
    We define $\mathsf{X} (\D)$ to be the set of all quadruples $(\eta, \kappa, g_0, \delta)$ where $\eta\from T \to \FF^\times$ is such that $\mathrm{d}\eta = \tilde \beta$ and $(\kappa, g_0, \delta)$ is an $\eta$-admissible triple. 
    Note that $\mathsf{X} (\D)$ can be identified with the set $\bigsqcup_{\vphi_0} \mathsf {X} (\D, \vphi_0)$. 
\end{defi}

By Corollary \ref{cor:iso-with-actions}, $(R, \vphi) \iso (R', \vphi')$ if, and only if, $(\eta', g_0', \delta', \kappa')$ is in the $(\D^\times_{\mathrm{gr}} \rtimes A ) \times G^\#$-orbit of $(\eta, \kappa, g_0, \delta)$, where the action of $(\D^\times_{\mathrm{gr}} \rtimes A ) \times G^\#$ is determined by its action on the isomorphism classes of $(\U, B)$ (see Lemma \ref{lemma:action-on-(U,B)}). 
Let us compute this action explicitly. 

Hence, let us consider these parameters fixed and focus on quadruples $(\eta, \kappa, g_0, \delta)$, where $(\kappa, g_0, \delta)$ is $\eta$-admissible. 
...
Let $t\in T$ and $0\neq X_t \in \D_t$ and let $(\eta', \kappa', g_0', \delta')$ be the parameters corresponding to $X_t \cdot (\U, B) = (\U, X_t B)$ (Equation \eqref{eq:Dx_gr-action}). 
To compute $\eta'$, recall that $dB$ is $(\mathrm{sInt}_d \circ \vphi_0)$-sesquilinear. 
Let $s\in T$ and $X_s \in \D_s$. 
Then
\begin{align*}
    (\mathrm{sInt}_{X_t} \circ \vphi_0) (X_s) &= \mathrm{sInt}_{X_t} ( \eta(s) X_s ) = \sign{t}{s} \eta(s) X_t X_s X_t\inv\\
    &= \sign{t}{s} \eta(s) \beta(t,s) X_s X_t X_t\inv = \tilde{\beta} (t,s) \eta(s) X_s
\end{align*}
and, hence, $\eta' (s) = \tilde{\beta} (t,s) \eta(s)$ for all $s\in T$. 
Since the action by $X_t$ does not change $\U$, $\kappa' = \kappa$. 
Clearly $g_0' = \deg dB = t g_0$. 
It remains to compute $\delta'$. 
Using Lemma \ref{lemma:bar-dB}, we have 
\begin{align*}
    \overline{X_t B} &= (-1)^{t} \vphi_0(X_t) \overline{B} = (-1)^{t} \eta(t) X_t \delta B  \\
    &= (-1)^{t} \eta(t) X_t \delta X_t\inv X_t B = (-1)^{t} \eta(t) \delta (X_t B),
\end{align*}
so $\delta' = (-1)^{t} \eta(t) \delta$. 
Note that we only need to know $t$ to compute $(\eta', \kappa', g_0', \delta')$, so we can consider a $T$-action instead of a $\D^\times_\mathrm{gr}$-action

Finally, we consider the $G^\#$-action. 
Let $g\in G$ and let $(\eta'', \kappa'', g_0'', \delta'')$ be the parameters corresponding to $g \cdot (\U, B) = (\U^{[g]}, B^{[g]})$ (Equation \eqref{eq:G-action}). 
...
Also, for all $u, v\ in \U$,
\begin{align*}
    \overline{B^{[g]}}(u,v) &= \sign{u^{[g]}}{v^{[g]}} \vphi_0\inv \big( B^{[g]} (v, u) \big) = (-1)^{(|g| + |u|) (|g| + |v|)} \vphi_0\inv \big( \sign{g}{v} B(v, u) \big) \\
    &= (-1)^{|g| + |g||u| + |g||v| + |u||v|} \sign{g}{v} \vphi_0\inv \big(B(v, u) \big) = (-1)^{|g| + |g||u|} \overline B(u,v) \\
    &= (-1)^{|g|} \sign{g}{u} \delta B(u,v) = (-1)^{|g|} \delta B^{[g]}(u,v),
\end{align*}
where $u^{[g]}$ and $v^{[g]}$ are the elements $u$ and $v$, respectively, but interpreted as elements of $\U^{[g]}$. 
We conclude that $\delta'' = (-1)^{|g|} \delta$. 

\begin{defi}
    We define a $T\times G^\#$-action on $\mathsf{X}(\D)$ by 
    \begin{align}
        t \cdot (\eta, \kappa, g_0, \delta) &\coloneqq (\eta \tilde \beta (t, \cdot), \kappa, t g_0, (-1)^{|t|} \eta(t)\delta)
        \intertext{and}
