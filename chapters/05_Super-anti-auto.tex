...
\begin{proof}
    The main fact we will use is that all division gradings on $(\mc E, \vphi)$ refining $\Gamma$ must have the same vector space decomposition. 
    To see that, let $\mc E_t$ denote the homogeneous component of degree $t$ of $\Gamma$, for every $t\in T$. 
    ...
    If $\Delta$ is a division grading, all the components are $1$-dimensional, so the components must be $\mc E_{(\delta, t)} \coloneqq $
