...
Let $(R, \vphi)$ be a superinvolution-simple superalgebra of type $M\times M\sop$ or $Q\times Q\sop$. 
...
If it is not graded-simple, the situation is easier: by \cref{prop:only-SxSsop-is-simple}, we have that $(R, \vphi)$


we have two alternatives: $R$ may be

Let $R \coloneqq S \times S\sop$ where $S$ is a finite dimensional simple superalgebra. 

In this section we will study the gradings on the superalgebras of the form 

Gradings on $R \coloneqq S \times S\sop$ are of two types. 
The type I gradings are the ones for which $R$ is not graded-simple. 
Since the nonzero proper superideals are $S \times \{ 0 \}$ and $\{ 0 \} \times S\sop$, at lest one of them must be a graded ideal. 
But $\vphi$ sends graded subsuperspace to graded subspaces, and, hence, one of them is graded if, and only if, both are. 
