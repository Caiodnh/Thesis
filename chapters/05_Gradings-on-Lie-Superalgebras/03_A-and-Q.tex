...
\begin{defi}\label{defi:type-II-A-m-not-n}
    ...
    \[
        L_{g} \coloneqq \{ s\in L_{\bar g} \mid \theta (s) = - \chi(g) s \},
    \]
    for all $g\in G$. 
\end{defi}
...
\begin{proof}
    ...
    \[
        \tilde u_i \coloneqq 
        \begin{cases}
            \sqrt{\chi(\deg u_i)\inv}, & \text{if }(\deg u_i) T = g_0\inv (\deg u_i)\inv T;\\
            \chi(\deg u_i)\inv u_i, & \text{if }(\deg u_i) T < g_0\inv (\deg u_i)\inv T;\\
