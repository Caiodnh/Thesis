...
Every such grading is isomorphic to $Q^{\mathrm{ex}} (T^+, \beta^+,  h, \kappa, g_0)$, where $T^+ \subseteq G$ is a finite $2$-elementary subgroup, $\beta^+\from T^+ \times T^+ \to \FF^\times$ is a bicharacter with $\rad \beta^+ = \langle f \rangle$ for a order $2$ element $f\in T^+$, $h \in G$ is an element such that $h^2 = f$
