...
\begin{defi}\label{defi:type-II-Q}
    ...
    Let $\barr \mu\from \barr T \to \FF^\times$ be the map determining the queer super transposition on $\barr \D$ (see \cref{ssec:param-D-vphi}), and 
...
\begin{proof}
    ...
    The choices in items (i) and (ii) give us a way to make the choices for a standard realization $(\D, \vphi_0)$ as in \ref{def:std-realization-MxM-QxQ}(c).
    Recall that the only choice for standard realization of type Q $\barr \D$ associated to $(\barr T^+, \bar \beta^+, h)$ (\cref{defi:type-II-Q}(i)) is the choice of standard realization associated to $(\barr T^+, \bar \beta^+)$ (see \cref{def:standard-realization-Q}).
    By the same argument as in \cref{prop:m-not-n-Type-II-correspondence}, this corresponds to a standard realization associated to $(K, \beta^+\restriction_{K \times K})$. 
