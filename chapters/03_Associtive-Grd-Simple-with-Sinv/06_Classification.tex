...
\begin{thm}\label{thm:iso-(R,vphi)-with-parameters}
	...
	Then $E(\D, \U, B) \iso E(\D', \U', B')$ if, and only if, $T = T'$, $\beta = \beta'$, $p = p'$, and $(\eta, \kappa, g_0, \delta)$ and $(\eta', \kappa', g_0', \delta')$ lie in the same orbit of the $T\times G^\#$-action on $\mathbf{I}(T, \beta, p)$ given in Definition \ref{def:TxG-action}. \qed
\end{thm}
...
% It is clear that the $T\times G^\#$-action on $\mathbf{I}(T, \beta, p)$ induces a $\mathcal G$-action on $\mathbf{I} (T, \beta, p, \eta)$. 

% Define $\mathbf{I}(T, \beta, p, \eta) \coloneqq \{ (\kappa, g_0, \delta) \mid (\eta, \kappa, g_0, \delta) \in \mathbf{I}(T, \beta, p)\}$. 
% Since the $T\times G^\#$-action can change the $\eta$ it does not restrict to the set $\mathbf{I}(T, \beta, p, \eta)$. 
% More explicitly, the 

% , but we can consider the action by the stabilizer of $\mathbf{I}(T, \beta, p, \eta)$, \ie, the action of the group $\mathcal{G} \coloneqq \rad \tilde\beta \times G^\#$
...
% We define the set $\mathbf{I}(T, \beta, p, \eta) \coloneqq \{ (\kappa, g_0, \delta) \mid (\eta, \kappa, g_0, \delta) \in \mathbf{I}(T, \beta, p)\}$. 
% Define $\mathcal{G} \coloneqq \rad \tilde\beta \times G^\#$. 
% Clearly, the action in Definition \ref{def:TxG-action} restricts to a $\mathcal{G}$-action on $\mathbf{I}(T, \beta, p, \eta)$.
