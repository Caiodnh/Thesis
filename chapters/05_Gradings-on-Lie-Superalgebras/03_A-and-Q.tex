...
We will the parameters $(T^+, \beta^+, h, \kappa)$ to construct a representative for the Type II $G$-grading in terms of the ``undoubled'' model, \ie, we will construct a $\barr G$-grading on $S$, where $\barr G \coloneqq G/\langle f \rangle$, and a super-anti-automorphism $\theta\from S\to S$, as in \cref{ssec:undoubling}. 
Let $\pi\from G \to \barr G$ denote the natural homomorphism, set $\barr {T^+} \coloneqq T^+/\langle f \rangle$ and set $\bar h \coloneqq \pi(h)$, let $\barr {\beta^+}\from \barr {T^+} \times \barr {T^+} \to \FF^\times$ be the (nondegenerate) bicharacter on $\barr {T^+}$ induced by $\beta^+$, and consider $\kappa$ as a map defined on $\barr G/\barr {T^+} \iso G/{T^+}$. 

We will proceed as in \cref{ssec:grds-on-A-m-n}, with $T^+$ playing the role of $T$ and $\beta^+$ playing the role of $\beta$. 
...
Choose a subgroup $K \subseteq T^+$ such that $T^+ = K \times \langle f \rangle$ (it can be done, since $T^+$ is a elementary $2$-group), and let $\chi\from T^+ \to \FF^\times$ be the character defined by $\chi(K) = 1$ and $\chi(f) = -1$. 
...
% A priori, the assertion that $\kappa$ is $g_0$-admissible depends on a map $\eta$ defined on the support of an odd graded-division superalgebra (\cref{defi:odd-D-kappa-g_0-admissible}(ii)). 
% Nevertheless, since $g_0\in G$ and $x\subseteq G$, the element $g_0g^2 \in T^+$, so we only need to know $\eta\restriction_{T^+}$. 
...
Let $\barr \D$ be a standard realization of type Q associated to $(\barr {T^+}, \bar\beta^+, \bar h)$ (compare with \cref{def:standard-realization-Q}). 
