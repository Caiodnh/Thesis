...
\begin{prop}\label{prop:self-dual-components}
    Suppose $x = g_0\inv x\inv$, let $\delta \in \pmone$ and  set $t \coloneqq g_0\xi(x)^2$. 
    Then the number 
    ...
    % \end{enumerate}
    Moreover, $B_x$ is nondegenerate and $\overline{B_x} = \delta B_x$ if, and only if, there is a $\D$-basis of $\U_{x}$ consisting only of elements of degree $\xi(x)$ such that the matrix representing $B_x$ is:
    \begin{enumerate}[(i)]
        \item $I_{\kappa(x)} \tensor X_t$ if $\mu_x = +1$;
        \item $J_{\kappa(x)} \tensor X_t$ if $\mu_{x} = -1$, where $J_{\kappa(x)} \coloneqq \begin{pmatrix}
        0 & I_{\kappa(x)/2}\\
        -I_{\kappa(x)/2} & 0
    \end{pmatrix}$ (in this case, $\kappa(x)$ must be even).
    \end{enumerate}
\end{prop}
...
\begin{proof}
    ...
    For the ``moreover'' part, it follows from Lemmas \ref{lemma:B_x-nondeg} and \ref{lemma:B_x-delta} that $B_x$ is nondegenerate and $\overline{B_x} = \delta B_x$ if, and only if, $\tilde B_x$ is nondegenerate and $B_x (u,v) = \mu_x B_x(v, u)$, for all $u,v \in \U_{\xi(x)}$. 
    Then the result follows from a well-known fact from linear algebra over algebraically closed fields and from the fact that an $\FF$-basis for $\U_{\xi(x)}$ is a $\D$-basis for $\U_x$.
\end{proof}
