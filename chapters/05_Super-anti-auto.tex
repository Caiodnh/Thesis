...
The next is a straightforward result about the concept of twist of a module (Definition \ref{def:twist}) in our current context.
...
We will now reduce the study of $B$ to the study of regular $\FF$-bilinear forms. 
Let $\xi\from G^\#/T \to G^\#$ be a set-theoretical section of the quotient homomorphism, \ie, $\xi (x) \in x$ for all $x \in G^\#/T$ and fix a nonzero element $X_t \in \D_t$ for all $t\in T$. 
Note that $\U_{\xi(x)} \tensor \D \iso \U_x$ via the map $u \tensor d \mapsto ud$ and, hence, a $\FF$-basis of $\U_{\xi(x)}$ is a graded $\D$-basis for $\U_x$. 

For a given $x \in G^\#/T$, set $y \coloneqq g_0\inv y\inv \in G^\#/T$ and $t \coloneqq g_0 \xi(x) \xi(y) \in T$. 
...
We will denote the restriction of $B$ to $\V_x$ by $B_x$ and define the bilinear map $B_x'\from V_x \times V_x \to \FF$ by $B_x' (u,v) \coloneqq B_x (u,v) X_{t}\inv$, for all $u,v \in V_x$. 

\begin{lemma}
    The $\vphi_0$-sesquilinear form $B_x$ is nondegenerate if, and only if, the bilinear form $B_x'$ is nondegenerate.
\end{lemma}

\begin{proof}
    Assume $B_x$ is nondegenerate and let $u \in V_x$ be such that $B_x'(u,v) = 0$ for all $v \in V_x$. 
    Then $B_x(u, v) = B_x'(u,v) X_{t} = 0$ for all $v \in V_x$ and, hence, $B_x(u, vd) = B_x(u, v)d = 0$ for all $v \in \V_x$. 
    It follows that $B_x(u,v) = 0$ for all $v\in \V_x = V_x \cdot \D$ and, therefore, $u = 0$.
    
    Now assume $B_x'$ is nondegenerate and let $u \in \V_x$ be such that $B_x (u,v) = 0$ for all $v \in \V_x$.
    Let $v\in V_x$ be homogeneous and write $u = \sum_{g\in G^\#} u_g$ where $u_g \in \U_g$. 
    Then $B_x(\sum_{g\in G^\#} u_g, v) = \sum_{g\in G^\#} B_x(u_g, v) = 0$ and, by degree considerations, $B_x(u_g, v) = 0$ for all $g\in G^\#$. 
    Since $\V_x = V \cdot \D$, we have that $u_g = u_g' d$ for some homogeneous elements $u_g' \in V_x$ and $0 \neq d \in \D$. 
    Then $B_x(u_g, v) = (-1)^{|d|(|B| + |u_g'|} \vphi_0(d) B_x(u_g', v) = 0$, and hence $B_x(u_g', v) = 0$. 
    We conclude that $B_x'(u_g', v) = 0$, and, since $v\in V_x$ was taken arbitrarily, $u_g' = 0$. 
    It follows that $u = 0$, concluding the proof.
\end{proof}

It is clear that $B$ is nondegenerate if, and only if, $B_x$ is nondegenerate for every $x \in G^\#/T$. 
In this case, if $\U_x$ and $\U_y$ are paired by $B$, then they are actually dual to each other and, hence, $\kappa(x) = \kappa(y)$. 
If $x = y$, we say that $\U_x$ is a \emph{self-dual component} and, if $x\neq y$, we say that $\U_x$ and $\U_y$ is a \emph{pair of different dual components}. 

% For the next result, we recall Definition ?? $\bar B$.

\begin{lemma}
    Let $\delta \in \FF$. 
    Then $\overline{B_x} = \delta B_x$ if, and only if, 
    \[
        B_x' (v, u) = (-1)^{|\xi(x)| |\xi(y)|} \eta(t) \delta B_x' (u, v)
    \] for all $u, v \in V_x$.
\end{lemma}
...
\begin{proof}
    ...
    \begin{alignat*}{3}
        ...
        &&= \sign{u}{v} \vphi_0\inv( B_x'(v, u)X_t )\\
        &= \sign{u}{v} B_x'(v, u) \vphi_0\inv(X_t)
        &&= \sign{u}{v} B_x'(v, u) \eta(t)\inv X_t.
    \end{alignat*}
    Hence, $\overline{B_x} (u, v) = \delta {B_x} (u, v)$ if, and only if,
    $
        \sign{u}{v} B_x'(v, u) \eta(t)\inv X_t = \delta B_x'(u, v) X_t
    $,
    and the result follows.
\end{proof}

From now on, we will assume that $\vphi_0$ is a superinvolution, \ie, that $\eta$ takes values in $\pmone$. 
Also, we will assume there is $\delta \in \pmone$ such that $\overline{B} = \delta B$ (see Theorem \ref{thm:vphi-involution-iff-delta-pm-1}). 


\begin{defi}
    Let $x \in G^\#$ and let $y \coloneqq g_0\inv x\inv$. 
    We define the number $\mu_x \in \pmone$ by
    \begin{enumerate}[(i)]
        \item 
    \end{enumerate}
\end{defi}
...
We will now focus our attention on the self-dual components. 




For convenience, we will fix a (set-theoretical) section of the natural projection map $G^\# \to G^\#/T$, \ie, a map $\xi\from G^\#/T \to G^\#$ such that $\xi(x) \in x$, for all $x\in G^\#/T$. 
If $\D$ is odd, then we choose $\xi$ to take even values, \ie, $\xi(x) \in G$, for all $x\in G^\#/T$. 
We will be also using the identification $M_k (\D) = \M_k(\FF) \tensor \D$ (see Remark \ref{rmk:M(D)=M(FF)-tensor-D}). 

% For convenience, we will also fix elements $0 \neq X_t \in \D_t$ for all $t\in T$.

\begin{lemma}\label{lemma:self-dual-components}
    Let $\U_{x}$ be a self-dual isotypic component of $\U$, and set $t \coloneqq g_0\xi(x)^2$ and $k \coloneqq \kappa(x)$. 
    % set 
    % $t \coloneq g_0g^2$ and 
    % $k \coloneqq \kappa(gT)$. 
    The number 
    \begin{equation}\label{eq:mu_x}
        \mu_{x} \coloneqq (-1)^{|\xi(x)|} \eta(t) \delta
    \end{equation}
    ...
    % \end{enumerate}
    If $\mu_{x} = +1$, then there is a $\D$-basis of $\U_{x}$ consisting only of elements of degree $\xi(x)$ such that the restriction of $B$ to $\U_{x}$ is represented by $I_k \tensor X_t$. 
    If $\mu_{x} = -1$, then $k$ is even and there is a $\D$-basis of $\U_{x}$ consisting only of elements of degree $\xi(x)$ such that the restriction of $B$ to $\U_{x}$ is represented by $J_k \coloneqq 
    \begin{pmatrix}
        0 & I_{k/2}\\
        -I_{k/2} & 0
...
\begin{remark}
    Even though $\mu_{x}$ does not depend on $\xi$, unless $T$ is a elementary $2$-group, the element $t = g_0\xi(x)^2$ does depend on it.
\end{remark}
...
\begin{lemma}
    ...
    \end{pmatrix} \tensor X_t.\]
    % where $t$ can be any element in $T$. 
    % If $\D$ is odd, then there are 
    % even 
    % graded $\D$-bases on $\U_{gT}$ and $\U_{g'T}$ such that the matrix representing the restriction of $B$ to $\U_{gT} \oplus \U_{g'T}$ is 
    % \[\begin{pmatrix}
    %     0 & I\\
    %     \delta I & 0
    % \end{pmatrix} \tensor X_t,\] where $t\in g_0 T^+$ can be taken arbitrarily. 
    % In both cases, we can take all elements of the basis of $\U_{gT}$ to have degree $g$ and all the elements of the basis of $\U_{g'T}$ to have degree $g'$.
\end{lemma}
...
\begin{defi}
    ...
    We say that the triple $(g_0, \delta, \kappa)$ is \emph{$(\D, \vphi_0)$-admissible} if:
    \begin{enumerate}[(i)]
...
% , where $g_0 = \deg B$, $\overline{B} = \delta B$ and $\kappa$ determines de isomorphism class of $\U$ as a graded $\D$-module. 
...
\begin{thm}
    ...
    % The map assigning a triple $(g_0, \delta, \kappa) \in \mathsf{X} (\D, \vphi_0)$ for ever
