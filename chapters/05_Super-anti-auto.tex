...
For convenience, we will be also using the identification $M_k (\D) = \M_k(\FF) \tensor \D$ (see Remark \ref{rmk:M(D)=M(FF)-tensor-D}). 
...
\begin{remark}
    Even though $\mu_{gT}$ does not depend on the choice of the representative $g \in G^\#$, unless $T$ is a elementary $2$-group, the element $g_0g^2 \in T$ does depend on it.
\end{remark}
