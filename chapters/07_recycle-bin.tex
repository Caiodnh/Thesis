...
\begin{defi}
    Ismorphism of pairs $(\D, \U)$.
\end{defi}

The following result (\cite[Theorem 2.10]{livromicha}) tell us when two such graded algebras are isomorphic:

\begin{thm}
    Let $R \coloneqq \End_\D(\V)$ and $R' \coloneqq \End_{\D'}(\V')$, where $\D, \D'$ are graded division superalgebras and $\V, \V'$ are nonzero right graded module of finite rank over $\D$ and $\D'$, respectively. 
    Given an isomorphism $\psi\from R \to R'$, there is $g_0 \in G$ and isomorphisms $\psi_0\from \D \to \D'$ and $\psi_1\from \V^{[g_0]} \to \V'$ such that
    \[
        \psi_1(rv) = \psi(r)\psi_1(r)
    \]
\end{thm}
