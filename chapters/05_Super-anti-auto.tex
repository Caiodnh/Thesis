...
We will now consider a nondegenerate homogeneous $\vphi_0$-sesquilinear form $B$ on $\U$, with degree $g_0 \in G^\#$.
...
From now on we will specialize our study to the case we are more interested. 
So we will assume that $\vphi_0$ is a superinvolution, \ie, that $\eta$ takes values in $\pmone$. 
...
For convenience, we will fix a representative $g\in G^\#$ for every coset $gT \in G^\#/T$, an ordered pair $(g', g'') \in (G^\#)^2$ for all $g', g'' \in G^\#$ such that $g_0 g' g'' \in T$ and a nonzero element $X_t \in \D_t$ for all $t\in T$. 
In the case $\D$ is odd, we will choose the coset representatives to be even elements. 
...
\begin{lemma}\label{lemma:self-dual-components}
    Let $\U_{gT}$ be a self-dual isotypic component of $\U$, and set $t \coloneqq g_0g^2$ and $k \coloneqq \kappa(gT)$. 
    % set 
    % $t \coloneq g_0g^2$ and 
    % $k \coloneqq \kappa(gT)$. 
    The number 
    \begin{equation}
        \mu_{gT} \coloneqq (-1)^{|g|} \eta(t) \delta
    \end{equation}
    ...
    % \end{enumerate}
    If $\mu_{gT} = +1$, then there is a graded $\D$-basis on $\U_{gT}$ such that the restriction of $B$ to $\U_{gT}$ is represented by $I_k \tensor X_t$. 
    If $\mu_{gT} = -1$, then $k$ is even and there is a graded $\D$-basis on $\U_{gT}$ such that the restriction of $B$ to $\U_{gT}$ is represented by $J_k \coloneqq 
    \begin{pmatrix}
        0 & I_{k/2}\\
        -I_{k/2} & 0
...
\begin{remark}
    Even though $\mu_{gT}$ does not depend on the choice of the representative $g \in G^\#$, unless $T$ is a elementary $2$-group, the element $t = g_0g^2$ does depend on it.
\end{remark}
...
\begin{lemma}
    ...
    If $\D$ is even, then there are graded $\D$-bases on $\U_{gT}$ and $\U_{g'T}$ such that the matrix representing the restriction of $B$ to $\U_{gT} \oplus \U_{g'T}$ is 
    \[\begin{pmatrix}
        0 & I_k\\
        \sign{g}{g'} \delta I_k & 0
    \end{pmatrix} \tensor X_t,\]
...
Clearly, to each pair $(\U, B)$, where $\U$ is graded $\D$-module of finite rank and $B$ is a nondegenerate $\vphi_0$-sesquilinear map such that $\overline B = \delta B$, we can a associate a $(\D, \vphi_0)$-admissible triple $(g_0, \delta, \kappa)$. 

Conversely, given a $(\D, \vphi_0)$-admissible triple $(g_0, \delta, \kappa)$, we can construct a pair $(\U, B)$. 

\begin{thm}
    Let $(\D, \vphi_0)$ be a finite dimensional graded-division superalgebra with superinvolution with $\D_e\even = \FF$. 
    The isomorphism classes of pairs $(\U, B)$, where are $\U$ is a graded $\D$-module of finite rank
