...
By \cref{thm:MxM-odd}, $(R, \vphi)$ endowed with such a grading is isomorphic to $M^{\mathrm{ex}} (T, \beta, t_p, \kappa, g_0)$ as defined in \cref{def:model-grd-MxM-odd-or-QxQ}. 
As we did in \cref{ssec:grds-on-Q(n)}, we will use parameters in terms of $G$ instead. 
...
\begin{proof}
    ...
    We already showed that $\eta\restriction_{T^+} = \eta^+$, hence, by \cref{eq:}, we have that $\barr\mu\restriction_{T^+}$, indeed, is the transposition on $\barr \D\even$. 
