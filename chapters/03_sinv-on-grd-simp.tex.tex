...
With this we can translate the problem of classifying $(\D, \vphi_0)$ to the level of abelian groups.
...
We will now discuss the connection between $\rad \beta$ and $\rad \tilde\beta$.
These subgroups of $T$ correspond to the support of the center and the supercenter of $\D$, respectively.
...
and that its isomorphism class is determined by the map $\kappa\from G^\#/T \to \ZZ_{\geq 0}$ with finite support (see \cref{sec:grd-D-modules}).
...
By the results above, the quadruple $(\eta, \kappa, g_0, \delta)$ satifies the following:
...
Given a quadruple $(\eta, \kappa, g_0, \delta) \in \mathsf{X} (\D)$, we can construct a pair $(\U, B)$ such that $(\eta, \kappa, g_0, \delta)$ is its inertia.
...
\begin{defi}\label{def:E(D,U,B)}
	...
	If $\D$ is associated to $(T, \beta, p)$ (see Subsection \ref{ssec:grd-div-alg}) and $(\U, B)$ has inertia $(\eta, \kappa, g_0, \delta) \in \mathsf{X}(T, \beta, p)$ (see Definitions \ref{def:parameter-of-(U,B)} and \ref{defi:X(D)}), then we say that $(T, \beta, p, \eta, \kappa, g_0, \delta)$ are the parameters of the triple $(\D, \U, B)$.
\end{defi}
...
Note that $(\eta', \kappa', g_0', \delta')$ depends only on $t$, so the $\D^\times_\mathrm{gr}$-action on $\mathsf{X}(T, \beta, p)$ factors through the action of $T \iso \D^\times_\mathrm{gr}/\FF^\times$.
...
\begin{defi}\label{def:TxG-action}
	The group $T\times G^\#$ acts on $\mathsf{X}(T, \beta, p)$ by
	\begin{align}
	...
	\end{align}
	for all $t\in T$, $g\in G^\#$ and $(\eta, \kappa, g_0, \delta) \in \mathsf{X}(T, \beta, p)$.
\end{defi}
...
We will now proceed to simplify our parameter set and the group acting on it.
...
We partition the set $\mathsf{X}(T, \beta, p)$ according to the equivalence class of $\eta$ and refer to $\{ (\eta', \kappa, g_0, \delta) \in \mathsf{X}(T, \beta, p) \mid \eta' \sim \eta \}$ as the $\eta$-block of the partition.
Note that if $(\eta, \kappa, g_0, \delta), (\eta', \kappa', g_0', \delta') \in \mathsf{X}(T, \beta, p)$ are in the same $T\times G^\#$-orbit, then, clearly, $\eta' \sim \eta$.
In other words, the $T\times G^\#$-action on $\mathsf{X}(T, \beta, p)$ restricts to each $\eta$-block of the partition.

We wish to fix $\eta$.
Given $\eta\from T \to \pmone$ such that $\mathrm{d}\eta = \tilde\beta$,
we define
\[
	\mathsf{X}(T, \beta, p, \eta) \coloneqq \{ (\kappa, g_0, \delta) \mid (\eta, \kappa, g_0, \delta) \in \mathsf{X}(T, \beta, p)\}.
\]
...
%     $(\eta, \kappa, g_0, \delta)$ is admissible, \ie, $(\eta, \kappa, g_0, \delta) \in \mathsf{X}(T, \beta, p)$. 
%     We denote the set of all $\eta$-admissible triples by $\mathsf{X}(T, \beta, p, \eta)$. 
%     In other words, $\mathsf{X}(T, \beta, p, \eta) \coloneqq \{ (\kappa, g_0, \delta) \mid (\eta, \kappa, g_0, \delta) \in \mathsf{X}(T, \beta, p)\}$. 
% \end{defi}
...
Thus, the $T\times G^\#$-action on $\mathsf{X}(T, \beta, p)$ induces an action of the subgroup $(\rad \tilde\beta) \times G^\#$ on $\mathsf{X}(T, \beta, p, \eta)$ given by
\begin{align}
...
\end{align}
for all $t\in \rad \tilde\beta$, $g\in G^\#$ and $(\kappa, g_0, \delta) \in \mathsf{X}(T, \beta, p, \eta)$.
...
We define
\[
	\mathsf{X}(T, \beta, p, \eta)^+ \coloneqq \{ (\kappa, g_0) \mid (\kappa, g_0, 1) \in \mathsf{X}(T, \beta, p, \eta)\}.
\]

% We proceed in a similar fashion as above. 
By the definition of the $(\rad \tilde\beta) \times G^\#$-action on $\mathsf{X}(T, \beta, p, \eta)$, we see that the action of $(t,g)$ does not change $\delta$ if, and only if, $
	(-1)^{|t|}\eta(t) = (-1)^{|g|}.
$
...
Thus, the $(\rad \tilde\beta) \times G^\#$-action on $\mathsf{X}(T, \beta, p, \eta)$ induces an $\mc G$-action on $\mathsf{X}(T, \beta, p, \eta)^+$ given by
\begin{align}
...
\end{align}
for all $(t, g)\in \mc G$ and $(\kappa, g_0) \in \mathsf{X}(T, \beta, p, \eta)^+$.

\begin{prop}
	Fix $\eta\from T \to \pmone$ such that $\mathrm{d}\eta = \tilde\beta$.
	We have that the map $\iota\from \mathsf{X}(T,\beta,p,\eta)^+ \to \mathsf{X}(T,\beta,p)$ given by $\iota(\kappa, g_0) \coloneqq (\eta,\kappa, g_0, 1)$ induces a bijection between the $\mc G$-orbits in $\mathsf{X}(T,\beta,p,\eta)^+$ and the $T\times G^\#$-orbits in the $\eta$-block of $\mathsf{X}(T,\beta,p)$.
\end{prop}

\begin{proof}
	If $(\kappa, g_0), (\kappa', g_0') \in \mathsf{X}(T, \beta, p, \eta)^+$ are in the same $\mc G$-orbit, let $(t, g) \in G \subseteq T\times G^\#$ such that $(t, g) \cdot (\kappa, g_0) = (\kappa', g_0')$.
	...
	So $\iota$ induces a function between the $\mc G$-orbits in $\mathsf{X}(T,\beta,p,\eta)^+$ and the $T\times G^\#$-orbits in the $\eta$-block of $\mathsf{X}(T,\beta,p)$.
	%
	\begin{itemize}
		\item \textit{Injectivity:}

		      % To check injectivity, 
		      Consider $(\kappa, g_0), (\kappa', g_0') \in \mathsf{X}(T, \beta, p, \eta)^+$ such that $\iota(\kappa, g_0)$ and $\iota(\kappa', g_0')$ are in the same $T\times G^\#$-orbit.
		...
		\item \textit{Surjectivity:}
		      ...
		      Let $(\eta', \kappa, g_0, \delta)$ bi in the $\eta$-block of $\mathsf{X}(T, \beta, p)$.
...
%     Let $(\eta', 
%     \kappa', g_0', \delta'), (\eta'', 
%     \kappa'', g_0'', \delta'') \in \mathsf{X}(T, \beta, p)$ such that $\eta'$ and $\eta''$ are equivalent to $\eta$, and let $t', t'' \in T$ be such that $\eta' = \beta(t', \cdot) \eta$ and $\eta'' = \beta(t'', \cdot) \eta$. 
%     Then $(\eta', 
...
Therefore, the classification up to isomorphism of $G$-graded superalgebras with superinvolution that are finite dimensional and graded-simple over an algebraically closed field $\FF$, $\Char \FF \neq 2$, reduces to the classification of finite subgroups $T\subseteq G^\#$, alternating bicharacters $\beta\from T\times T \to \pmone$, equivalence classes of $\eta$ (see \cref{def:equiv-eta}) and $\mathcal G$-orbits in $\mathsf{X}(T,\beta, p, \eta)^+$.
...
% Since the $T\times G^\#$-action %of Definition \ref{def:TxG-action} 
% can change $\eta$, we have to restrict our group to do that. 
% Clearly, the $T\times G^\#$-action induces an $T$-action on the set $\{ \eta \from T \to \FF^\times \mid \mathrm{d} \eta = \tilde \beta \}$, and the stabilizer of this action in any point $\eta$ is $\rad \tilde\beta = (\rad \beta) \cap T^+$ (see Lemma \ref{lemma:rad-tilde-beta}). 
% This motivates the following:

% \begin{defi}
%     We will denote the group $\rad \tilde\beta \times G^\#$ by $\mathcal{G}$. 
% \end{defi}

% It is clear that the $T\times G^\#$-action on $\mathsf{X}(T, \beta, p)$ induces a $\mathcal G$-action on $\mathsf{X} (T, \beta, p, \eta)$. 

% Define $\mathsf{X}(T, \beta, p, \eta) \coloneqq \{ (\kappa, g_0, \delta) \mid (\eta, \kappa, g_0, \delta) \in \mathsf{X}(T, \beta, p)\}$. 
% Since the $T\times G^\#$-action can change the $\eta$ it does not restrict to the set $\mathsf{X}(T, \beta, p, \eta)$. 
% More explicitly, the 

% , but we can consider the action by the stabilizer of $\mathsf{X}(T, \beta, p, \eta)$, \ie, the action of the group $\mathcal{G} \coloneqq \rad \tilde\beta \times G^\#$

% We define an equivalence relation $\sim$ on set of all maps $\eta \from T \to \FF^\times$ such that $\mathrm{d} \eta = \tilde \beta$ by declaring  $\eta \sim \eta'$ if there is $t\in T$ such that $\eta' = \tilde\beta (t, \cdot) \eta$. 
