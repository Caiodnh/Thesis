...
\begin{proof}
    Given $t_1 \in S$, its is straightforward that $t_1' \in S$ \IFF $t_1\inv t_1' \in \rad \beta^+$. 
...
\[\label{eq:eta+-unsharpening-MxM}
    \begin{aligned}
        \mathrm{d}\eta^+ = \beta^+,\, \eta^+\restriction_{K} \,\, &\text{is the map associated to transposition on}\, \mc M,\,\AND\,
        \eta^+(f) = -1. 
    \end{aligned}
\]
