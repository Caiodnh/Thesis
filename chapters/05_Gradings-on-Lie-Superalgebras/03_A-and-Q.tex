...
\begin{defi}\label{def:type-II-P}
    Let $n\geq 2$. 
    Let $T \subseteq G$ be a finite $2$-elementary subgroup, let $\beta\from {T\times T} \to \FF^\times$ be an alternating bicharacter with $\rad \beta = \langle f \rangle$, for some order $2$ element $f\in T$, let $\kappa_\bz \from G/T \to \ZZ_{\geq 0}$ be a map with finite support, and let $h_0 \in G$ be any element. 
    ...
    \end{enumerate}
    Let $\barr \mu\from \barr T \to \FF^\times$ be the map determining the transposition on $\barr \D$ (see \cref{ssec:param-D-vphi}), 
    let $\chi \in \widehat{T}$ be the character such that $\chi(K) = 1$ and $\chi(f) = -1$, and extend $\chi$ to a character on $\widehat{G}$, which we will also denoted by $\chi$. 
    ...
    Also, set $\gamma_\bo = = (h_0\inv g_1\inv, \ldots, h_0\inv g_k\inv)$ (recall that $g_0$ is odd and \cref{inertia-even-and-odd-case}).   
    Consider the $\barr G$-grading $\Gamma_M(\barr T, \barr \beta, \kappa_\bz, \kappa_\bo)$ on $S \coloneqq M(n+1,n+1)$ using the choices above (see \cref{def:Gamma-T-beta-kappa-even}), and consider the induced grading $\Gamma : L = \bigoplus_{\bar g \in \barr G} L_{\bar g}$ to $L \coloneqq S^{(1)}/Z(S^{(1)})$. 
    Consider ${\Theta \in S}$ 
    given by
    ...
    \cref{eq:theta-with-matrix-3}. 
    We define $\Gamma_A^{\mathrm{(II)_P}}(T, \beta, \kappa_\bz, h_0)$ to be the $G$-grading on $L$ given by
    \[
        L_{g} \coloneqq \{ s\in L_{\bar g} \mid s = - \chi(g) s \},
    \]
    for all $g\in G$. 
