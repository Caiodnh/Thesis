...
\begin{thm}\label{thm:iso-D-even}
	...
	Then $\End_\D (\U) \iso \End_{\D'} (\U')$ if, and only if, $T=T'$, $\beta=\beta'$, $p = p'$ and there is $g\in G$ such that either $g \kappa_{\bar 0}=\kappa_{\bar 0}'$ and $g \kappa_{\bar 1}=\kappa_{\bar 1}'$ or $g\kappa_{\bar 0}=\kappa_{\bar 1}'$ and $g \kappa_{\bar 1}=\kappa_{\bar 0}'$. \qed
% 	\begin{enumerate}[(i)]
...
It should be noted, in the case $\Char \FF \neq 2$, that \cref{prop:simple-R-D-super,cor:tilde-beta-nondeg} together imply that $\End_\D (\U)$ is simple as a superalgebra if, and only if, $\tilde\beta$ is nondegenerate. 
Nevertheless, even in the case $\Char \FF = 2$, we can use \cref{thm:iso-D-even,thm:iso-D-odd} to get a classification of abelian group gradings on finite dimensional simple superalgebras, which we write as 3 different resuts:

% ------------------------------------

\begin{itemize}
    \item Need to odd gradings on $M(m,n)$ only if $m = n$, before;
    \item elementary grading on $M(m,n)$, define somewhere;
    \item start with $Q$?
    \item Definitions for $M(m,n)$ even and odd. 
    \item on odd, recall the convention?
\end{itemize}

\begin{defi}\label{def:Gamma-T-beta-kappa-even}
    Let $m, n\in \ZZ_{\geq 0}$ such that $m+n > 0$. 
    Given a finite subgroup $T \subseteq G$, a nondegenerate bicharacter $\beta\from T\times T \to \FF^\times$ and maps $\kappa_\bz, \kappa_bo \from G/T \to \ZZ_{\geq 0}$ with finite support such that $|\kappa_\bz| \sqrt{|T|} = m$ and $|\kappa_\bo| \sqrt{|T|} = n$, consider:
    \begin{enumerate}[(i)]
        ...
        \item the elementary grading (see \cref{defi:elementary-grd}) on $M(k_\bz, k_\bo)$ defined by a $k_\bz + k_\bo$-tuple $\gamma = (g_1, \ldots, g_{k_\bz}, \ldots, g_{k_\bz + k_\bo})$ of elements of $G$, $k_\bz \coloneqq |\kappa_\bz|$ and $k_\bo \coloneqq |\kappa_\bo|$, such that, for every $x\in G/T$, the number of entries $g_i$ with $g_i\in x$ and $1\leq i \leq k_\bz$ is equal to $\kappa_\bz (x)$, and  the number of entries $g_i$ with $g_i\in x$ and $k_\bz < i \leq k_\bz + k_\bo$ is equal to $\kappa_\bo (x)$.
    \end{enumerate}
    %
    We define $\Gamma (T, \beta, \kappa_\bz, \kappa_\bo)$ to be the even grading on $M(m,n)$ given by identifying $M(m,n)$ with the graded algebra $M(k_\bz, k_\bo) \tensor \D$ via Kronecker product, \ie,
    \[
...
\begin{defi}\label{def:Gamma-T-beta-kappa-odd}
    ...
    Given a finite subgroup $T \subseteq G^\#$, $T\not\subsetneq G$, a nondegenerate bicharacter $\beta\from T\times T \to \FF^\times$ and a map $\kappa\from G/T^+ \to \ZZ_{\geq 0}$ with finite support such that $|\kappa| \sqrt{|T|} = n$, consider 
    \begin{enumerate}
        \item the group homomorphism $p\from T \to \ZZ_2$ given by the projection on the second component of $G^\# = G\times \ZZ_2$;
        \item a standard realization $\D$ (see \cref{def:standard-realization}) of a matrix algebra with a division grading associated to $(T,\beta)$, viewed as a $G$-graded superalgebra;
    ...
    \end{enumerate}
    %
    We define $\Gamma (T, \beta, p, \kappa)$ to be the grading on $M(n,n)$ given by identifying $M(n,n)$ with the graded superalgebra $M_k(\FF) \tensor \D$ via Kronecker product, \ie,
    \[
        \deg \left( E_{ij} \tensor d \right) = g_ig_j\inv t,
    \] 
...
\begin{defi}\label{def:Gamma-T-beta-kappa-odd}
    Let $n > 0$ be a natural number. 
    Given a finite subgroup $T^+ \subseteq G$, a nondegenerate bicharacter $\beta^+ \from T^+ \times T^+ \to \FF^\times$, element $h \in G$ such that $h^2 = e$ and a map $\kappa\from G/T^+ \to \ZZ_{\geq 0}$ with finite support such that $|\kappa| \sqrt{|T^+|} = n$, consider 
    \begin{enumerate}
    ...
    \end{enumerate}
    %
    We define $\Gamma (T^+, \beta^+, h, \kappa)$ to be the grading on $Q(n)$ given by identifying $Q(n)$ with the graded superalgebra $M_k(\FF) \tensor \D$ via Kronecker product, \ie,
    \[
        \deg \left( E_{ij} \tensor d \right) = g_ig_j\inv t,
    \] 
    for all $1\leq i, j \leq k$, $t\in T$ and $0 \neq d \in \D_t$.
