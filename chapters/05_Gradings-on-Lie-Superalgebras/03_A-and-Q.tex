...
We will first construct a grading on $S\even$. 
Recall that $S = Q(n+1)$ can be identified with $Q(1) \tensor M_{n+1}(\FF)$ via Kronecker product (where we are using the associative $Q(1)$), so have an identification of $S\even$ with $M_{n+1}(\FF) = M(n+1, 0)$. 

We proceed as in \cref{ssec:grds-on-A-m-n}, with $T^+$ playing the role of $T$ and $\beta^+$ playing the role of $\beta$. 
Note that 
