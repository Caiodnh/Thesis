...
\begin{proof}
    ...
    Therefore, all the data in the description of the undoubled model of $\Skew(R, \vphi)$ coincide with with the data in \cref{defi:type-II-A-m-not-n}, concluding the proof.
\end{proof}
...
\begin{proof}
    ...
    By \cref{lemma:barr-D-to-mc-M}, with $(T^+, \beta^+)$ playing the role of $(T, \beta)$, the choices of $K$ and $\barr \D\even$  give us the same information as the choices in  of $K$ in item \eqref{item:K-can-be-orthogonal-to-t_1} and $\mc M$ in item \eqref{item:choose-mc-M} of \cref{def:std-realization-MxM-QxQ}(c), and the map associated to the transposition on $\mc M$ is $\mu^+\restriction_K$. 
    ...
    We have already showed that $\eta\restriction_{T^+} = \eta^+$ and, hence, by \cref{eq:fix-eta-plus-undouble-Q}, we have that $\barr\mu\restriction_{\barr{T^+}} = \barr{\mu^+}$.
...
\begin{remark}
    A $G$-grading on the Lie superalgebra $\osp(n+1,n+1)$ (respectively, $P(n)$) gives rise to a $G \times \ZZ_2$-grading on $A(n,n)$ of Type II\textsubscript{osp} (respectively, II\textsubscript{P}). 
\end{remark}
