...
To do that we will assume that $\D$ is fixed
A priori, the isomorphism class of $(R, \vphi)$ depend on $(\D, \vphi_0, \U, B)$


In Section ??, we described graded-simple associative superalgebras with super-anti-automorphism with the data $(\D, \vphi_0, \U, B)$ and Theorem ?? tell us when two such superalgebras with super-anti-automorphism are isomorphic. 
We can interpret this result with the language of group actions. 
To do that, though, we will fix the graded-division superalgebra $\D$. 
Note that if $\psi_0\from \D \to \D'$ is an isomorphism, then we can use it to transfer a quadruple $(\D', \vphi_0', \U', B')$ to $(\D, \vphi_, \U, B)$ by defining $\vphi_0 \coloneqq \psi_0\inv \circ \vphi_0' \circ \psi_0$, $\U = \U^{\psi_0}$ and $B \coloneqq \vphi_0\inv B'$. 
Also, recall that since $B$ is nondegenerate, it determines $\vphi_0$. 
Hence, once we fixed $\D$, only the data $(U, B)$ matters.

Theorem tell us we can change this data in 3 ways. 
Given a homogeneous $0 \neq d\in D$, we can consider $( \vphi_0\circ \mathrm{sInt}_{d\inv}, \U, dB)$. 
In other words, this gives an action of the group $\D_{\mathrm{gr}}^\times$.

If $\psi_1\from \U \to \U$ is an automorphism, then we can consider $ $

Theorem \ref{thm:vphi-iff-vphi0-and-B} tell us that graded-simple associative superalgebras with super-anti-automorphism can be recored, up to isomorphims, 
