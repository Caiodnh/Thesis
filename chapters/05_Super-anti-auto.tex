...
\begin{thm}\label{thm:refinement-DxDsop}
	...
	\item If $\tilde \Delta$ is another refinement with $\supp \tilde \Delta = T_\Delta$ and ${}^\pi \tilde \Delta = \Gamma$, then there is a uinque selfequivalence $\alpha$ of $\{ \pm 1 \} \xrightarrow{\iota} T_\Delta \xrightarrow{\pi} T$ such that $\tilde \Delta = {}^\alpha \Delta$. \label{item:number-of-extensions}
    \end{enumerate}
    %  
    %Also, 
    %
...
\begin{cor}
    Under the conditions of Theorem \ref{thm:refinement-DxDsop}, the set of all refinements $\Delta$ such that $\supp \Delta = E$ and ${}^\pi \Delta = \Gamma$ is in bijection with the group homomorphisms from $T$ to $\{ \pm 1 \}$.
\end{cor}
...
The graded division superalgebra $\mc O$ is of special importance among the odd graded division superalgebras whose underlying superalgebra is of the form $M(m,n) \times M(m,n)\sop$.

Of course, the construction of the superalgebra $\mc O$ depend on the choice of the involution on $\D$. 
Using item \eqref{item:number-of-extensions} of Theorem \ref{thm:refinement-DxDsop} and Corrolary, there are $4$ other superalgebras we could end up with if we took different choices (since there are $4$ homomorphisms $\ZZ_2 \times \ZZ_2 \to \ZZ_2$). 
...
\begin{thm}
    ...
    Then there are graded-division superalgebras with involution $\D'$ and $\D''$ such that $\D \iso \D' \otimes \D''$ as graded superalgebras with involution, $\D''$ is simple as an algebra (and, hence, even) and
    \begin{enumerate}[(i)]
        \item if $\D \iso M(m,n)\times M(m,n)\sop$ as superalgebras, then $\D' \iso {}^\alpha\mc O$, where $\mc O$ is given in Example ?? and $\alpha\from $;
