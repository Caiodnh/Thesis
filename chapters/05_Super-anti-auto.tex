...
\begin{proof}
    ...
    Recall that for a division grading, its support is its universal group (Lemma ??). 
    ...
    From the fact that the extensions are equivalent, if we put $\Delta' = \Delta_0$, then Item \eqref{item:equivalent-extension-example} follows from Lemma ??. 
    ...
    Since this extension is equivalent to the one corresponding to $\pmone \times_{\tilde\beta} T$, we can apply Lemma ?? to conclude that the self-equivalences are in bijection with group homomorphisms from $T$ to $\pmone$, concluding the proof.
