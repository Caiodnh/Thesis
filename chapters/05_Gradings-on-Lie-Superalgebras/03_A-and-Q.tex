...
\begin{proof}
    ...
    % Need: $\D$ is what it should be
    Note that 
    
    % Constructs $(\U, B)$ using the construction in osp, by reducing it to $(\U\even, B\even)$
    ...
    Note that $\B$ is an even graded $\D$-basis of $\U$, and let $B\from \U \times \U \to \D$ the unique $\vphi_0$-sesquilinear extension of $B\even$,\ie, $B$ is also represented by $\Phi$. 
    % We now write $\Phi$ with entries in $\D$ in a smart way, putting $Q(1)$ in the front.
    
    % From \Phi we get \Theta
    To get the matrix $\Theta$, we proceed as in \cref{prop:m-not-n-Type-II-correspondence}. 
    We consider a different graded basis $\tilde B = \{ \tilde u_1, \ldots, \tilde u_k \}$, where $\tilde u_i$ is defined as in \cref{eq:tilde-u_i-from-u_i}, and let $\tilde \Phi$ be the matrix representing $B$ with respect to $\tilde \B$. 
    Following the procedure in \cref{ssec:undoubling} with $\tilde \Phi$ playing the role of $\Phi$, we get $\Theta$ as in \cref{eq:Theta-for-Q}. 
