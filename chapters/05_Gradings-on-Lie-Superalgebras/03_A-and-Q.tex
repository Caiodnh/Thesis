...
Gradings of Type II\textsubscript{osp} are construct exactly like in the previous subsection. 
The definition is the same. 

\begin{defi}\label{def:type-II-osp}
    If $m = n$ in \cref{defi:type-II-A-m-not-n}, we will denote $\Gamma(...)$ by $\Gamma()$... .
\end{defi}

The proof it works is also the same. 

To construct a model for gradings of Type II\textsubscript{P}, we follow a similar approach as for Type II\textsubscript{osp}, but it is simpler since we do not need to choose $\xi$ and $\leq$ to define the matrix $\Theta$. 
This is the same reason why it is simpler to construct models for gradings on $P(n)$ (ssec or def?) than it is to construct gradings on $\osp(m,n)$ (ssec). 

We choose $\chi$ as before, and put $\Theta$ as what it should be, the same diagonal matrix as blocks in the secondary diagonal.
